...
class SessionAggregatorRegistry:
    """Holds the lazy ``SessionAggregator`` and ``SecurityReportService``.

    State that previously lived in module-level ``_session_aggregator``
    and ``_security_report_service`` globals is now carried as
    instance attributes on a process-wide ``_REGISTRY`` singleton, so
    the shared-state contract of the original module is preserved
    while the helpers move to plain instance methods.
    """

    def __init__(self) -> None:
        self._session_aggregator: SessionAggregator | None = None
        self._security_report_service: SecurityReportService | None = None

    def get_session_aggregator(self) -> SessionAggregator:
        ...
        """
        if self._session_aggregator is None:
            self._session_aggregator = self._build_session_aggregator()
        return self._session_aggregator

    def set_session_aggregator(self, agg: SessionAggregator | None) -> None:
        """Override (or clear) the shared aggregator. For tests only."""
        self._session_aggregator = agg

    def get_security_report_service(self) -> SecurityReportService:
        """Return the shared ``SecurityReportService``, lazy-built."""
        if self._security_report_service is None:
            self._security_report_service = (
                self._build_security_report_service()
            )
        return self._security_report_service

    def set_security_report_service(
        self, svc: SecurityReportService | None,
    ) -> None:
        """Override (or clear) the shared reports service. For tests only."""
        self._security_report_service = svc

    def reset(self) -> None:
        """Clear every cached singleton — used by tests between cases."""
        self.set_session_aggregator(None)
        self.set_security_report_service(None)

    # ------------------------------------------------------------------
    # Builders
    # ------------------------------------------------------------------

    def _build_session_aggregator(self) -> SessionAggregator:
        base_providers, history = self._optional_user_service_parts()
        ...
        extras = self._build_security_session_providers()
        ...
        return SessionAggregator(
            ...
            known_usernames=self._known_usernames_from_user_store,
        )

    def _build_security_session_providers(self) -> list:
    ...
    def _known_usernames_from_user_store(self) -> list[str]:
    ...
    def _build_security_report_service(self) -> SecurityReportService:
        audit, _providers, history = self._user_service_parts()
        return SecurityReportService(
            audit_log=audit,
            session_aggregator=self.get_session_aggregator(),
            login_history=history,
        )

    def _optional_user_service_parts(self) -> tuple[list, Any | None]:
        """Return (providers, login_history), or ``([], None)`` when the
        user-service factory isn't available (reduced-footprint deploys
        and most unit tests).
        """
        try:
            _audit, providers, history = self._user_service_parts()
        except Exception as exc:  # noqa: BLE001
            _log.debug(
    ...
    def _user_service_parts(self) -> tuple[Any, list, Any]:
        ...
        history = LoginHistoryIndex(
            audit_log=audit, session_store=session_store,
        )
        try:
            history.rebuild()
        except Exception as exc:  # noqa: BLE001
...
# Process-wide singleton — preserves the shared-state contract of the
# previous module-level globals. Tests reach in via the ``set_*`` /
# ``reset`` aliases below, identical to the legacy API.
_REGISTRY = SessionAggregatorRegistry()

get_session_aggregator = _REGISTRY.get_session_aggregator
set_session_aggregator = _REGISTRY.set_session_aggregator
get_security_report_service = _REGISTRY.get_security_report_service
set_security_report_service = _REGISTRY.set_security_report_service
reset = _REGISTRY.reset


__all__ = [
    "SessionAggregatorRegistry",
