...
# Sensible role → service-id defaults so direct_hosts works even
# when no profile YAML is mounted (the K8s common case before the
# user has saved any profile-level config). technology_bindings
# overrides when present.
_ROLE_DEFAULTS = {"media_server": "jellyfin", "auth": "authelia"}


class K8sIngressSyncService:
    """Reconcile the K8s Ingress resource from runtime routing config.

    All helpers live as instance methods so callers (jobs, API
    handlers, tests) can either drive a single shared instance or
    construct their own. Module-level aliases at the bottom of the
    file expose the same names the previous loose-function API used.
    """

    def routing_from_runtime(self) -> dict[str, Any]:
        """Read the merged routing config (profile + overrides) the
        same way generate_envoy_config_main reads it. Single source
        of truth so envoy-config and ingress-config can't disagree on
        what the user asked for."""
        from media_stack.api.services.config import get_routing
        return dict(get_routing() or {})

    def enabled_web_ui_services(self) -> list[tuple[str, int]]:
        """List of (service_id, internal_port) for every service with
        a web UI. We only emit ingress rules for these — backend-only
    ...
    def build_rules(
        ...
        TLS-host scope rule (added v1.0.163 after Let's Encrypt
        rate-limit near-miss): only OPERATOR-DECLARED hostnames go on
        the TLS list, because cert-manager auto-creates a Certificate
        from ``spec.tls.hosts`` and tries to issue a Let's Encrypt
        cert for each. Putting per-service subdomains there means
        cert-manager asks LE to validate ``sonarr.example.com`` etc.,
        which fails (no DNS), spawns one solver pod per host, and
        burns through the LE rate-limit. Operator-declared =
        ``gateway_host`` + every value in ``direct_hosts``.
        Per-service subdomains stay as INGRESS RULES (so envoy
        receives the request if DNS happens to point there) but skip
        TLS — works over HTTP, or the operator can add their own
        multi-SAN cert.
        """
        rules: list[dict] = []
        tls_hosts: list[str] = []
        ...
        # path-prefix or subdomain inside its own vhost config).
        # Required for path-prefix and hybrid strategies.
        if gateway_host and strategy in ("hybrid", "path-prefix", "subdomain"):
            self._append_rule(rules, tls_hosts, seen, gateway_host, "envoy", 80)

        # Per-service subdomains — only when strategy includes
        # subdomain routing AND the operator has provided enough info
        # to build the FQDN (need both stack_subdomain and
        # base_domain, otherwise we'd silently emit ``<svc>.local``
        # and stomp the LAN fallback set).
        if strategy in ("subdomain", "hybrid") and stack_subdomain and base_domain:
            for svc_id, _port in services:
                ...
                # Subdomain rules go to envoy too — envoy routes by
                # Host header to the right backend. Bypassing envoy
                # here would skip auth/middleware policies.
                #
                # request_tls=False: per-service subdomains are NOT
                # automatically added to the TLS hosts list. Most
                # operators don't have public DNS for every
                # per-service subdomain (they pick a small set, like
                ...
                self._append_rule(
                    rules, tls_hosts, seen, host, "envoy", 80, request_tls=False,
                )

        # Direct hosts — bypass envoy for specific services
        # (typically the media server, where TV/mobile clients prefer
        # a simple host without path-prefix gymnastics). Maps each
        # direct_host to the named service if the service is in the
        # enabled set.
        ...
        for role, host in (direct_hosts or {}).items():
            ...
            #                  entry for the auth provider —
            #                  auth.provider in profile YAML names
            #                  it).
            #
            # Anything else is tried as a literal service id, so
            # ``direct_hosts: {sonarr: srn.example.com}`` works
            # without a binding indirection.
            if role == "auth":
                svc_id = (
                    (profile_data.get("auth") or {}).get("provider")
                    or _ROLE_DEFAULTS["auth"]
                )
            else:
                svc_id = bindings.get(role) or _ROLE_DEFAULTS.get(role) or role
            port = svc_port_map.get(svc_id)
            if port:
                self._append_rule(rules, tls_hosts, seen, host_str, svc_id, port)

        # LAN fallback: <svc>.local → envoy. Always emitted so an
        # operator without DNS can use /etc/hosts to reach apps.
        # These don't go on the TLS hosts list because LAN access is
        # plain HTTP via the default ingress; the operator can opt in
        # by adding them.
    ...
    def patch_ingress(
        ...
        """Apply the desired rules/TLS to the live Ingress via the
        K8s API."""
        try:
        ...
        # Collect the existing TLS secret name so we don't overwrite
        # a cert-manager-managed secret with our default just because
        # the operator never told us. Falls back to the env var or
        # default.
    ...
    def reconcile(self) -> dict[str, Any]:
        ...
        PATCH the Ingress. Safe to call repeatedly (idempotent: if
        the rules already match, the PATCH is a no-op at the K8s API
        level).

        On K8s, this is the FIRST-HOP control plane: without rules
        in the Ingress, external traffic never reaches Envoy in the
        first place. That means "reconcile produced nothing" is not
        a silent skip — it is a hard failure of the clean-deploy
        invariant. This function ``raise``s rather than returning
        ``skipped`` when routing looks incomplete on a K8s cluster;
        bootstrap surfaces the exception in the dashboard instead of
        quietly proceeding to a broken state.

        Exception: when the controller is NOT running on K8s
        (``K8S_NAMESPACE`` unset), this is a genuine no-op — compose
        has no Ingress layer to reconcile and the caller is just
        invoking the job unconditionally from the DAG. Returning a
        skip is correct there and matches ``ingress-config``'s
        contract phase 21 semantics.

        All inner helper calls dispatch through ``sys.modules`` so
        tests that ``mock.patch`` ``_routing_from_runtime`` /
        ``_enabled_web_ui_services`` / ``_build_rules`` /
        ``_patch_ingress`` at module level still intercept.
        """
        ...
        routing = _self_module._routing_from_runtime()
        if not routing:
            raise RuntimeError(
        ...
        services = _self_module._enabled_web_ui_services()
        rules, tls_hosts = _self_module._build_rules(routing, services)
        if not rules:
            raise RuntimeError(
        ...
        return _self_module._patch_ingress(namespace, rules, tls_hosts)


_INSTANCE = K8sIngressSyncService()

# Module-level aliases — keep the previous loose-function API
# working for callers (jobs, handlers) and tests that ``mock.patch``
# these names directly.
_routing_from_runtime = _INSTANCE.routing_from_runtime
_enabled_web_ui_services = _INSTANCE.enabled_web_ui_services
_build_rules = _INSTANCE.build_rules
_patch_ingress = _INSTANCE.patch_ingress
reconcile = _INSTANCE.reconcile
