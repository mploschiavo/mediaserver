...
This module follows ADR-0012: a single ``RoutingMigrator`` class
holds the logic as plain instance methods, and module-level aliases
are bound to a singleton instance so existing callers and tests can
``mock.patch`` via ``sys.modules[__name__]``. Tested via
...
class RoutingMigrator:
    """Container for routing v1 → v2 migration helpers (ADR-0012).
    ...
    def is_v2(self, d: dict[str, Any]) -> bool:
        if _coerce_int(d.get("version"), 1) >= 2:
            return True
        return any(k in d for k in _V2_KEYS)

    def resolve_service_id(self, role: str, media_server_id: str | None) -> str:
        ...
        media-server tech via the profile; we replicate that here when
        the caller provides ``media_server_id``. Roles we don't
        recognise are treated as literal service IDs (the v1 behaviour).
        """
        if role == "media_server":
            return media_server_id or "jellyfin"
        if role == "auth":
            # The auth role can be authelia or other providers; pick
            # the canonical name. Validation will catch a non-existent
            # service.
            return "authelia"
        return role

    def hosts_from_direct_hosts(
        self,
        direct_hosts: Any,
        media_server_id: str | None,
        auth_gate_default: AuthGate,
    ) -> list[HostEntry]:
        """Convert the v1 ``direct_hosts: {role: hostname}`` dict to a
        list of ``HostEntry``. Empty/falsy hostnames are dropped — they
        were the v1 way to "unset" a role without removing the key."""
        out: list[HostEntry] = []
        ...
        for role, hostname in direct_hosts.items():
            ...
            if not host_str:
                continue
            service_id = self.resolve_service_id(
                _coerce_str(role), media_server_id,
            )
            # The auth provider must never gate itself with itself,
            # else operators get locked out. v1 implicitly enforced
            # this; v2 makes it explicit.
            gate = (
    ...
    def migrate_v1_to_v2(
        ...
        *,
        ...
        Args:
            raw: the dict as loaded from ``routing.yaml`` (or
                empty/None).
            media_server_id: profile's media-server selection
                (jellyfin, plex, …) — used when migrating
                ``direct_hosts.media_server`` to a concrete service_id.
                ``None`` falls through to ``jellyfin``.
        ...
        cfg = RoutingConfigV2(
            ...
            hosts=self.hosts_from_direct_hosts(
        ...
        # If the gateway_host wasn't already in any host's
        # canonical/aliases, populate exposure.public_hostnames so the
        # v2 view has a starting point. v1 implicitly treated
        # gateway_host as "the public face"; losing that on migration
...
_INSTANCE = RoutingMigrator()
_is_v2 = _INSTANCE.is_v2
_resolve_service_id = _INSTANCE.resolve_service_id
_hosts_from_direct_hosts = _INSTANCE.hosts_from_direct_hosts
migrate_v1_to_v2 = _INSTANCE.migrate_v1_to_v2
