...
class RoutingConfigValidator:
    """Pure-data validator for ``RoutingConfigV2``.

    No I/O, no network, no global state — every input flows through
    method args. Tests construct one validator and feed it different
    configs (see ``tests/unit/api/services/config/routing/test_validator.py``).
    """
    ...
    def validate(
        ...
        *,
        ...
        for i, p in enumerate(cfg.path_aliases):
            if not self.is_path(p.from_path):
                errors.append(ValidationError(
                    code="VR-4",
        ...
        if apex.action == ApexAction.REDIRECT:
            ...
            elif not self.is_path(apex.target):
                errors.append(ValidationError(
                    code="VR-5",
                    field="apex.target",
        ...
        if catch_all.action == CatchAllAction.REDIRECT:
            ...
            elif not self.is_path(catch_all.target):
                errors.append(ValidationError(
                    code="VR-6",
                    field="catch_all.target",
...
_INSTANCE = RoutingConfigValidator()

# Module-level aliases preserve the legacy import API:
# ``from .validator import validate_routing_config`` is the only
# documented entry point, used by ``api/routes/post_config_writes.py``
# and ``api/routes/routing_admin.py``. ``_is_path`` is exposed for any
# future patch-by-name test hook.
validate_routing_config = _INSTANCE.validate
_is_path = _INSTANCE.is_path
