...
The function is pure: takes optional path / now / platform overrides
...
def read_state(
    *,
    ...
    """Build the response payload for ``GET /api/orchestrator/promises/state``.

    Returns ``(status_code, body)``:

      * ``(200, payload)`` when the persisted file exists and was
        written within ``stale_threshold_seconds``.
      * ``(503, payload)`` when the file is missing, malformed, or
        older than the threshold. The 503 body still carries
        ``last_tick_age_seconds`` (or ``None`` if no saved_at) so
        the verifier can choose to retry without reparsing the body.

    All three orchestrator-runtime fields (``platform``,
    ``live_services``, the cooldown file path) can be overridden for
    tests; the defaults read from the same env / FS the orchestrator
    itself reads.
    """
    ...
    if live_services is None:
        ...
        live_services = _live_services_from_env() or frozenset()
    ...
    if not state_path.is_file():
        return 503, {
            ...
            "platform": platform,
            "live_services": sorted(live_services),
        }
    ...
    try:
        ...
        return 503, {
            "error": f"orchestrator state unreadable: {exc.__class__.__name__}",
            "saved_at": None,
            "last_tick_age_seconds": None,
            "platform": platform,
            "live_services": sorted(live_services),
    ...
    saved_at = float(saved_at_raw) if isinstance(saved_at_raw, (int, float)) else None
    age = (now_seconds - saved_at) if saved_at is not None else None

    if saved_at is None or age is None or age > stale_threshold_seconds:
        return 503, {
    ...
    for promise_id in sorted(attempts_dict):
        ...
        attempts_list.append({
            "promise_id": str(entry.get("promise_id") or promise_id),
            "status": attempt_status,
            "started_at": float(entry.get("started_at") or 0.0),
            "elapsed_seconds": float(entry.get("elapsed_seconds") or 0.0),
            "detail": str(entry.get("detail") or ""),
            "probe_evidence": dict(entry.get("probe_evidence") or {}),
            "ensurer_fired": bool(entry.get("ensurer_fired") or False),
            "ensurer_attempts": int(entry.get("ensurer_attempts") or 0),
    ...
    body = {
        ...
        "totals": {
            ...
            "failed_transient": status_counts.get("failed_transient", 0),
            "failed_permanent": status_counts.get("failed_permanent", 0),
            "dep_failed": status_counts.get("dep_failed", 0),
            "skipped_cooldown": status_counts.get("skipped_cooldown", 0),
            "skipped_platform": status_counts.get("skipped_platform", 0),
