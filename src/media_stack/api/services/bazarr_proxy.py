...
class BazarrProxyService:
    """Service wrapping Bazarr's REST API for the dashboard's subtitle
    preferences card. Methods are plain instance methods; module-level
    aliases below dispatch through ``sys.modules[__name__]`` so tests
    that ``mock.patch`` the public names hit the live binding.
    """

    def _bazarr_get(self, path: str) -> Any:
    ...
    def _bazarr_post(self, path: str, body: dict[str, Any]) -> Any:
        api_keys = discover_api_keys()
        key = api_keys.get("bazarr", "")
        if not key:
            raise RuntimeError(
    ...
    def _bazarr_patch(self, path: str, body: dict[str, Any]) -> Any:
    ...
    def get_subtitle_config(self) -> dict[str, Any]:
        ...
        # module-level _bazarr_get alias and have it observed here.
        ...
        try:
            langs = mod._bazarr_get("/api/system/languages")
        ...
        try:
            profiles = mod._bazarr_get("/api/system/languages/profiles")
            if isinstance(profiles, list):
                ...
                try:
                    settings = mod._bazarr_get("/api/system/settings")
...
_INSTANCE = BazarrProxyService()

# Module-level aliases — every public name exposed pre-refactor stays
# importable. Tests `mock.patch` these names; methods that call into
# these helpers re-resolve via `sys.modules[__name__]` so the patches
# are honoured.
_bazarr_get = _INSTANCE._bazarr_get
_bazarr_post = _INSTANCE._bazarr_post
_bazarr_patch = _INSTANCE._bazarr_patch
get_subtitle_config = _INSTANCE.get_subtitle_config
update_subtitle_languages = _INSTANCE.update_subtitle_languages
