...
def validate_authelia_config(config: Any) -> list[ValidationError]:
    ...
    if isinstance(cookies, list):
        for idx, cookie in enumerate(cookies):
            errors.extend(
                _validate_cookie(idx, cookie if isinstance(cookie, dict) else {})
            )
...
def _validate_cookie(idx: int, cookie: dict) -> list[ValidationError]:
    errors: list[ValidationError] = []
    ...
    for url_key in ("authelia_url", "default_redirection_url"):
        ...
        if not host:
            continue
        if not _host_under_domain(host, domain):
            errors.append(ValidationError(
...
def validators_for(
    ...
    return {sid: _VALIDATORS[sid] for sid in service_ids if sid in _VALIDATORS}
