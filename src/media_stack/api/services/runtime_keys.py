...
class RuntimeKeysService:
    """Resolves service API keys with env→file fallback and TTL caching.

    Plain instance methods only (per ADR-0012). Module-level aliases at
    the bottom expose the public surface; tests that ``mock.patch`` a
    name should patch the module attribute, and lookups dispatch through
    ``sys.modules[__name__]`` so the patch is honored.
    """
    ...
    def _read_env_key(self, service_id: str) -> str:
    ...
    def _read_file_key(self, service_id: str) -> str:
    ...
    def read_service_api_key(self, service: str) -> Optional[str]:
        ...
        if not service:
            return None
        # Dispatch helpers through the module so tests that ``mock.patch``
        # ``runtime_keys._now`` / ``_read_env_key`` / ``_read_file_key``
        # keep working after the class refactor.
        ...
        env_val = mod._read_env_key(service)
        if env_val:
            result: Optional[str] = env_val
        else:
            file_val = mod._read_file_key(service)
    ...
    def services_missing_keys(self) -> list[str]:
        ...
        try:
            ...
            log_swallowed(exc)
            return missing
        # Dispatch through the module so a ``mock.patch`` of
        # ``runtime_keys.read_service_api_key`` is honored.
        mod = sys.modules[__name__]
        for svc in SERVICES:
            ...
            try:
                ...
                log_swallowed(exc)
            if mod.read_service_api_key(svc.id) is None:
                missing.append(svc.id)
        return missing


_INSTANCE = RuntimeKeysService()

# Module-level aliases — public surface stays identical to the pre-ADR-0012
# free-function shape so callers and ``mock.patch`` targets don't shift.
_now = _INSTANCE._now
_read_env_key = _INSTANCE._read_env_key
_read_file_key = _INSTANCE._read_file_key
read_service_api_key = _INSTANCE.read_service_api_key
invalidate_cache = _INSTANCE.invalidate_cache
services_missing_keys = _INSTANCE.services_missing_keys
