...
class ConfigRootDiscovery:
    """Encapsulates CONFIG_ROOT auto-discovery strategies."""

    # ------------------------------------------------------------------
    # Shared: container name -> service ID mapping
    # ------------------------------------------------------------------

    def _build_container_map(self, containers: list[Any]) -> dict[str, Any]:
    ...
    def _discover_via_docker_mounts(self, log: Any = None) -> DiscoveryResult:
        ...
        container_map = sys.modules[__name__]._build_container_map(containers)
    ...
    def _discover_via_docker_env(self, log: Any = None) -> DiscoveryResult:
    ...
    def discover_config_root(
        ...
        try:
            mount_result = mod._discover_via_docker_mounts(log=log)
        ...
        try:
            env_result = mod._discover_via_docker_env(log=log)
        ...
        if not merged.config_root:
            try:
                scan_result = mod._discover_via_path_scan(current_root, log=log)
...
_INSTANCE = ConfigRootDiscovery()

# Module-level aliases for every public/private name (preserves import surface).
_build_container_map = _INSTANCE._build_container_map
_discover_via_docker_mounts = _INSTANCE._discover_via_docker_mounts
_discover_via_docker_env = _INSTANCE._discover_via_docker_env
_discover_via_path_scan = _INSTANCE._discover_via_path_scan
discover_config_root = _INSTANCE.discover_config_root
