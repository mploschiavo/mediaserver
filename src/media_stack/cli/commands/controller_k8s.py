...
class ControllerK8sSecretWriter:
    """Patches discovered API keys + user IDs into the
    ``media-stack-secrets`` K8s Secret.
    ...
    Module-level ``_persist_preflight_keys_to_secret`` alias is
    bound to the singleton's method so callers + the
    ``controller_main`` re-export keep working.
    """

    def _persist_preflight_keys_to_secret(self, state: object) -> None:
        """Patch discovered API keys from preflights into the K8s
        secret.

        The controller discovers keys via HTTP during preflights, but
        downstream services (and reconcile CronJobs) need them in the
        K8s secret.
        """
    ...
    def _collect_env_keys(self, string_data: dict[str, str]) -> None:
        ...
        subprocess stub)."""
        # Lazy import: avoids a controller_main → controller_k8s →
        # core.service_registry → controller_main cycle on eager load.
    ...
    def _patch_secret(
        ...
        """Issue the actual ``patch_namespaced_secret`` call with
        base64-encoded values. Wraps the kubernetes client load + patch
        in a single broad ``except`` because the optional ``kubernetes``
        package may not be installed on dev / compose hosts."""
        try:
