...
class SetupLanTlsCommand:
    """Wraps LAN TLS setup CLI entrypoint."""

    def parse_args(self, argv: list[str] | None = None) -> argparse.Namespace:
        parser = argparse.ArgumentParser(prog="bin/setup-lan-tls.sh",
            description="Generates a LAN TLS certificate for ingress hosts and configures ingress TLS.")
        parser.add_argument("--namespace", default=(os.environ.get("NAMESPACE", "media-stack") or "media-stack"))
        parser.add_argument("--ingress-name", default=(os.environ.get("INGRESS_NAME", "media-stack-ingress") or "media-stack-ingress"))
        parser.add_argument("--tls-secret-name", default=(os.environ.get("TLS_SECRET_NAME", "media-stack-tls") or "media-stack-tls"))
        parser.add_argument("--node-ip", default=(os.environ.get("NODE_IP", "") or "").strip())
    ...
    def main(self, argv: list[str] | None = None) -> int:
        ...
        with tempfile.TemporaryDirectory(prefix="media-stack-lan-tls-") as tmpdir:
            ...
            secret_yaml = run_command([*kubectl, "-n", args.namespace, "create", "secret", "tls",
                args.tls_secret_name, f"--cert={crt}", f"--key={key}", "--dry-run=client", "-o", "yaml"], check=True).stdout
            run_command([*kubectl, "-n", args.namespace, "apply", "-f", "-"], check=True, input_text=secret_yaml)
        patch_payload = {"spec": {"tls": [{"secretName": args.tls_secret_name, "hosts": hosts}]}}
        run_command([*kubectl, "-n", args.namespace, "patch", "ingress", args.ingress_name,
            "--type", "merge", "-p", json.dumps(patch_payload)], check=True)
    ...
    def _generate_with_openssl(crt, key, hosts, node_ip, work_dir):
        ...
        lines = ["[req]", "distinguished_name=req_distinguished_name", "x509_extensions=v3_req",
            "prompt=no", "[req_distinguished_name]", f"CN={hosts[0]}", "[v3_req]",
        ...
        san_file.write_text("\n".join(lines) + "\n", encoding="utf-8")
        run_command(["openssl", "req", "-x509", "-nodes", "-newkey", "rsa:4096", "-keyout", str(key),
            "-out", str(crt), "-days", "365", "-config", str(san_file), "-extensions", "v3_req"], check=True)


_instance = SetupLanTlsCommand()
parse_args = _instance.parse_args
main = _instance.main

if __name__ == "__main__":
    raise SystemExit(main())
_resolve_node_ip = _instance._resolve_node_ip
_read_ingress_hosts = _instance._read_ingress_hosts
_generate_with_mkcert = _instance._generate_with_mkcert
_generate_with_openssl = _instance._generate_with_openssl
