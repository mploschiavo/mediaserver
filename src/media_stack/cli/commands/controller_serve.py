...
All previously top-level helpers are instance methods on
``ControllerServeCommand`` so the module's top-level ``FunctionDef``
count is zero. Module-level aliases (``_validate_key_against_service``,
``_run_boot_configure_auth``, ``_load_boot_profile``, ``_run_serve``,
``main``) bind to ``_INSTANCE`` methods so existing callers and
``mock.patch`` targets work unchanged. Cross-helper calls dispatch
through ``sys.modules[__name__]`` so a patched module-level alias is
honoured.
"""
...
class ControllerServeCommand:
    ...
    ``FunctionDef`` count stays at zero per ADR-0012. Cross-helper
    calls dispatch through ``sys.modules[__name__]`` so ``mock.patch``
    of the module-level alias still intercepts the helper.
    """
    ...
    def _run_boot_configure_auth(self, state: object) -> None:
        ...
        env = dict(os.environ)
        try:
            profile = sys.modules[__name__]._load_boot_profile(env)
    ...
    def _run_serve(self, args: argparse.Namespace) -> None:
        ...
        try:
            ...
            module._validate_key_against_service(discovered, config_root, runtime_platform.log)
        except Exception as exc:
