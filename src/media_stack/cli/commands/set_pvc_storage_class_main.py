...
def _find_spec_block_end(lines: list[str], spec_idx: int, spec_indent: int) -> int:
    ...
    while idx < len(lines):
        ...
        if stripped and _indent_width(lines[idx]) <= spec_indent:
            break
        idx += 1
    return idx
...
def _set_storage_class(lines: list[str], class_name: str) -> list[str]:
    updated = _remove_storage_class(lines)
    spec_idx = _find_spec_index(updated)
    if spec_idx < 0:
        return updated

    spec_indent = _indent_width(updated[spec_idx])
    block_end = _find_spec_block_end(updated, spec_idx, spec_indent)
...
def split_yaml_documents(text: str) -> list[str]:
    if not text.strip():
        return [""]
    parts = DOC_SEPARATOR_RE.split(text)
    return parts
...
def transform_storage_class_manifest(text: str, class_name: str, clear_mode: bool) -> str:
    ...
    for doc in split_yaml_documents(text):
        ...
        if not _is_pvc_document(lines):
            transformed_docs.append(doc.strip("\n"))
            continue

        if clear_mode:
            result_lines = _remove_storage_class(lines)
        else:
            result_lines = _set_storage_class(lines, class_name)
        transformed_docs.append("\n".join(result_lines).strip("\n"))

    return render_yaml_documents(transformed_docs)
...
def parse_config(argv: list[str] | None = None) -> SetStorageClassConfig:
    ...
    parser = build_arg_parser(root_dir / "deploy" / "k8s" / "storage-pvc.yaml")
    ...
    return SetStorageClassConfig(target_file=target, class_name=class_name, clear_mode=clear_mode)
