...
from media_stack.adapters.compose.controller_service import ComposeBootstrapConfig, ComposeBootstrapService
from media_stack.infrastructure.platforms.compose.docker_client import DockerClient
from media_stack.adapters.compose.rebuild_platform_adapter import (
...
class ComposePlugin:
    """Compose platform plugin bindings as cohesive class methods."""

    def docker_client(self, runner: object) -> DockerClient:
    ...
    def build_adapter(self, request: object, require_dependency) -> object:
        ...
        return ComposeRebuildPlatformAdapter(
            cfg=ComposeRebuildPlatformConfig(
                ...
                app_gateway_port=request.app_gateway_port,
    ...
    def build_runner_request(self, runner: object, info_fn) -> dict[str, object]:
        ...
        return {
            ...
            "info": info_fn,
            "docker_client": module._docker_client(runner),
            ...
            "app_gateway_port": runner.cfg.app_gateway_port,
    ...
    def run_bootstrap(self, runner: object) -> None:
        ...
        service = ComposeBootstrapService(
            ...
            info=runner.info_fn,
            docker=module._docker_client(runner),
        )
        service.run()


_INSTANCE = ComposePlugin()

_docker_client = _INSTANCE.docker_client
