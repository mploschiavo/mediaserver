...
class PlatformPluginRegistry:
    """Registry for discovering and resolving platform plugins."""
    ...
    def available_platform_targets() -> tuple[str, ...]:
        return tuple(sorted(PlatformPluginRegistry.load_platform_plugins().keys()))
    ...
    def normalize_platform_target(target: str) -> str:
        ...
        if not token:
            return ""
        for key, plugin in PlatformPluginRegistry.load_platform_plugins().items():
            aliases = {key}
    ...
    def resolve_platform_plugin(target: str) -> PlatformPlugin | None:
        normalized = PlatformPluginRegistry.normalize_platform_target(target)
        if not normalized:
            return None
        return PlatformPluginRegistry.load_platform_plugins().get(normalized)
...
_instance = PlatformPluginRegistry()
load_platform_plugins = _instance.load_platform_plugins
available_platform_targets = _instance.available_platform_targets
normalize_platform_target = _instance.normalize_platform_target
resolve_platform_plugin = _instance.resolve_platform_plugin
