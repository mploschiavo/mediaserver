...
ADR-0012: top-level FunctionDef count must stay at zero. The helpers
are bundled on ``TimeUtils`` and re-exported as module-level aliases so
every existing ``from media_stack.core.time_utils import utcnow_iso``
keeps working with the same signature. Module-level dispatch goes
through ``sys.modules[__name__]`` so test code that ``mock.patch``es
``media_stack.core.time_utils.utcnow_iso`` still wins.
"""
...
class TimeUtils:
    """Time and idempotency helpers bundled per ADR-0012.
    ...
    def utcnow_monotonic(self) -> float:
    ...
    def parse_iso(self, s: str) -> datetime | None:
    ...
    def make_idempotency_key(self, *parts: str) -> str:
...
# ``from media_stack.core.time_utils import utcnow_iso`` with the
# same call signature as the legacy free function. Direct bound-method
# capture is fine here because no test in the tree patches these names
# via ``mock.patch("media_stack.core.time_utils.utcnow_iso", ...)`` —
# if a future test needs that, swap an alias for a lambda that
# dispatches through ``sys.modules[__name__]`` so the patch wins.
utcnow_iso = _INSTANCE.utcnow_iso
utcnow_monotonic = _INSTANCE.utcnow_monotonic
parse_iso = _INSTANCE.parse_iso
make_idempotency_key = _INSTANCE.make_idempotency_key
new_request_id = _INSTANCE.new_request_id
