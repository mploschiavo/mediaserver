...
def _match_cli(ua_lower: str) -> tuple[str, str] | None:
...
def _match_tv(ua_lower: str) -> tuple[str, str] | None:
...
def _match_jellyfin(ua_lower: str) -> tuple[DeviceClass, str, str] | None:
...
def _match_phone(ua_lower: str) -> tuple[str, str] | None:
    ...
    if os_family is None:
        return None
    return (os_family, _detect_browser(ua_lower))


def _match_tablet(ua_lower: str) -> tuple[str, str] | None:
...
def _match_desktop(ua_lower: str) -> tuple[str, str] | None:
...
def classify(user_agent: str | None) -> DeviceFingerprint:
    ...
    ua_lower = _lower(stripped)

    cli = _match_cli(ua_lower)
    if cli is not None:
        os_family, app = cli
...
def classify_class(user_agent: str | None) -> DeviceClass:
    ...
    return classify(user_agent).device_class
