...
class K8sIngressAdapter:
    ...
    def _all_public_hostnames(self, cfg: RoutingConfigV2) -> list[str]:
    ...
    def _cert_for_host(self, host: HostEntry, cfg: RoutingConfigV2) -> CertEntry | None:
    ...
    def _issuer_annotation(self, cert: CertEntry) -> dict[str, str]:
    ...
    def _build_ingress(self, cfg: RoutingConfigV2) -> dict[str, Any]:
        ...
        hostnames = self._all_public_hostnames(cfg)
        ...
        for c in cfg.certs:
            if c.source == CertSource.CERT_MANAGER:
                annotations.update(self._issuer_annotation(c))
            ...
            rules.append({
                ...
                "http": {
                    "paths": [{
                        ...
                        "backend": {
                            "service": {
                                "name": self._backend_service(),
                                "port": {"number": self._backend_port()},
                            },
                        },
                    }],
                },
    ...
    def _build_service_patch(self, cfg: RoutingConfigV2) -> dict[str, Any]:
    ...
    def _build_certificate(self, c: CertEntry, hostnames: list[str]) -> dict[str, Any] | None:
    ...
    def compute_apply_plan(self, cfg: RoutingConfigV2) -> ApplyPlan:
        ...
        plan.steps.append(ApplyPlanStep(
            kind="service.patch",
            description=(
                f"Set {self._backend_service()} Service type → "
                f"{self._build_service_patch(cfg)['spec']['type']}"
            ),
            payload=self._build_service_patch(cfg),
        ))

        if cfg.exposure.enabled and hostnames:
            ingress = self._build_ingress(cfg)
            plan.steps.append(ApplyPlanStep(
                kind="ingress.apply",
                description=(
            ...
            for c in cfg.certs:
                cert_obj = self._build_certificate(c, hostnames)
            ...
            plan.steps.append(ApplyPlanStep(
                ...
                payload={
                    "apiVersion": "networking.k8s.io/v1",
                    "kind": "Ingress",
                    "metadata": {"name": "media-stack-edge", "namespace": self._ns()},
                },
            ))
...
# top-level `def`s.
_adapter = K8sIngressAdapter()
_ns = _adapter._ns
_ingress_class = _adapter._ingress_class
_backend_service = _adapter._backend_service
_backend_port = _adapter._backend_port
_all_public_hostnames = _adapter._all_public_hostnames
_cert_for_host = _adapter._cert_for_host
_issuer_annotation = _adapter._issuer_annotation
_build_ingress = _adapter._build_ingress
_build_service_patch = _adapter._build_service_patch
_build_certificate = _adapter._build_certificate
