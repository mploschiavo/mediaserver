"""Filesystem cleanup helpers for media hygiene operations."""
...
def walk_existing_files(paths: list[Path]):
    for root in paths:
        if not root.exists():
            continue
...
def run_filesystem_hygiene(ops, hygiene_cfg: dict[str, Any]) -> dict[str, int]:
