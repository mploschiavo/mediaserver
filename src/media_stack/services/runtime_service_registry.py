...
class RuntimeServiceRegistry:
    """Manages runtime adapter-hook context and service-class resolution."""

    def __init__(self) -> None:
        self._runtime_context_adapter_hooks: dict[str, object] = {}

    def set_runtime_context_cfg(self, adapter_hooks_cfg=None) -> None:
        self._runtime_context_adapter_hooks = dict(adapter_hooks_cfg or {})

    def get_runtime_context_cfg(self) -> dict[str, object]:
        if not isinstance(self._runtime_context_adapter_hooks, dict):
            return {}
        return dict(self._runtime_context_adapter_hooks)

    def get_runtime_binding(self, binding_key: str, default: str = "") -> str:
        key = str(binding_key or "").strip()
        if not key:
            return str(default or "")
        ctx = self.get_runtime_context_cfg()
    ...
    def _active_adapter_hooks(self) -> dict[str, object]:
        if isinstance(self._runtime_context_adapter_hooks, dict) and self._runtime_context_adapter_hooks:
            return dict(self._runtime_context_adapter_hooks)
        return self._manifest_adapter_hooks()
    ...
    def resolve_app_service_class(self, service_key, default_cls, technology: str = ""):
        key = str(service_key or "").strip()
        if not key:
            return default_cls

        hooks = self._active_adapter_hooks()
        ...
        if str(technology or "").strip():
            ...
            if isinstance(by_technology_map, dict):
                canonical_technology = self._canonical_technology_key(technology, hooks)
        ...
        return self._load_class_from_spec(
            spec,
...
_instance = RuntimeServiceRegistry()
set_runtime_context_cfg = _instance.set_runtime_context_cfg
