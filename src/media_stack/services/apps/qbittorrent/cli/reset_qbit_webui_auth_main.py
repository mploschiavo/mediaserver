...
class ResetQbitWebuiAuthCommand:
    def parse_config(self, argv: list[str] | None = None) -> ResetQbitWebuiAuthConfig:
    ...
    def locate_conf_path(
        self, kubectl: list[str], cfg: ResetQbitWebuiAuthConfig
    ) -> str:
        proc = run_command(
            [
                *kubectl,
...
""",
    ...
    def wait_until_no_pods(
        self, kubectl: list[str], cfg: ResetQbitWebuiAuthConfig, seconds: int = 60
    ) -> None:
    ...
    def run(self, cfg: ResetQbitWebuiAuthConfig) -> int:
        kubectl = kube_cmd()
        print(f"[INFO] Locating qBittorrent config file inside deploy/{cfg.deployment}")
        module = sys.modules[__name__]
        conf_path = module.locate_conf_path(kubectl, cfg)
        if not conf_path:
...
""",
        ...
        module.wait_until_no_pods(kubectl, cfg)
...
_INSTANCE = ResetQbitWebuiAuthCommand()
parse_config = _INSTANCE.parse_config
locate_conf_path = _INSTANCE.locate_conf_path
wait_until_no_pods = _INSTANCE.wait_until_no_pods
run = _INSTANCE.run
main = _INSTANCE.main
