...
def discover_api_key(config_root: str) -> str:
...
def discover_admin_user_id(base_url: str, api_key: str, username: str) -> str:
...
def reset_password(
    ...
    if not jf_key:
        jf_key = discover_api_key(config_root)
        if jf_key:
            os.environ["JELLYFIN_API_KEY"] = jf_key
    if jf_key and not jf_uid:
        jf_uid = discover_admin_user_id(jf_base, jf_key, username)
        if jf_uid:
...
def hard_reset(username: str, password: str) -> dict[str, Any]:
