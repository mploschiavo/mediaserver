...
class JellyfinAdapters:

    def normalize_provider_name(self, value: str) -> str:
        return re.sub(r"[^a-z0-9]+", "", str(value or "").strip().lower())

    def reorder_provider_names(
        self, names: Iterable[str], priority_terms: Iterable[str] | None = None
    ) -> List[str]:
        ...
        for raw in names or []:
            ...
            if not name:
                continue
            key = self.normalize_provider_name(name)
            if key in seen:
                continue
            seen.add(key)
            deduped.append(name)

        priorities = [self.normalize_provider_name(term) for term in (priority_terms or []) if term]
        if not priorities or not deduped:
            return deduped

        chosen: List[str] = []
        used = set()
        normalized = [(name, self.normalize_provider_name(name)) for name in deduped]
...
_instance = JellyfinAdapters()
normalize_provider_name = _instance.normalize_provider_name
reorder_provider_names = _instance.reorder_provider_names
apply_artwork_profile = _instance.apply_artwork_profile
