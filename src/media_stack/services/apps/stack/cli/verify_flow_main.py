    ...
    def _check_homepage_hosts(self) -> None:
        ...
        hp_services = self._kube_stdout(
            [
                ...
                "sh",
                "-lc",
                "cat /app/config/services.yaml 2>/dev/null",
            ]
        )
...
""".strip()
    ...
    def run(self) -> int:
        ...
        for app in ("Sonarr", "Radarr", "Lidarr", "Readarr"):
            self._check_log(
                rf"Prowlarr: updated application link for {app}", f"{app} <- Prowlarr app link"
            )
            self._check_log(
                rf"{app}: (updated|created|reconciled existing named) qBittorrent download client",
                f"{app} -> qBittorrent client wired",
            )
            self._check_log(
                rf"{app}: (updated|created|reconciled existing named) SABnzbd download client",
                f"{app} -> SABnzbd client wired",
                optional=True,
            )
            self._check_log(
                rf"{app}: remote path mapping (created|updated|already set)",
                f"{app} SAB remote path mappings reconciled",
                optional=True,
            )
            self._check_log(
                rf"{app}: discovery list reconcile complete",
                f"{app} discovery lists reconciled",
                optional=True,
            )
            self._check_log(
                rf"{app}: (updated media management|media management already set).*hardlinks=True",
                f"{app} hardlinks policy enforced",
            )
            self._check_log(
                rf"{app}: (updated download handling|download handling already set)",
                f"{app} CDH enabled",
            )
            self._check_log(
                rf"{app}: (updated download handling|download handling already set).*removeFailed=True.*autoRedownloadFailed=True",
                f"{app} self-healing failed-download retry enabled",
            )

        self._check_log(
            r"Jellyseerr: configured Jellyfin connection", "Jellyseerr -> Jellyfin wired"
        )
        self._check_log(
            r"Jellyseerr: (updated|created|existing) (Radarr|Sonarr)",
            "Jellyseerr -> Arr mappings wired",
        )
        ...
        self._check_log(
            r"Jellyfin playback: reconcile complete",
            "Jellyfin playback defaults reconciled",
            optional=True,
        )
        self._check_log(
            r"Jellyfin home rails: reconcile complete",
            "Jellyfin curated home rails reconciled",
            optional=True,
        )
        self._check_log(
            r"Homepage: (wrote services config|services config already up-to-date)",
            "Homepage services config reconciled",
            optional=True,
        )
        self._check_log(
            r"Bazarr: (wrote integration config|Sonarr/Radarr integration already matches desired config|Sonarr/Radarr \+ subtitle automation config already matches desired state)",
            "Bazarr Sonarr/Radarr integration reconciled",
            optional=True,
        )
        ...
        self._check_writable(
            ...
            "Radarr can write /data/torrents/completed/movies",
        )
        self._check_writable("sonarr", "/media/tv", "Sonarr can write /media/tv")
        self._check_writable(
            "sonarr", "/data/torrents/completed/tv", "Sonarr can write /data/torrents/completed/tv"
        )
        ...
        self._check_writable(
            ...
            "qBittorrent can write category path /data/torrents/completed/tv",
        )
        self._check_writable(
            "sabnzbd", "/data/usenet/completed", "SABnzbd can write /data/usenet/completed"
        )
        self._check_writable(
            "sabnzbd", "/data/usenet/completed/tv", "SABnzbd can write /data/usenet/completed/tv"
        )
        self._check_writable(
            "sabnzbd",
            "/data/usenet/completed/movies",
            "SABnzbd can write /data/usenet/completed/movies",
        )
        self._check_writable(
            "sabnzbd",
            "/data/usenet/completed/music",
            "SABnzbd can write /data/usenet/completed/music",
        )
        self._check_writable(
            "sabnzbd",
            "/data/usenet/completed/books",
            "SABnzbd can write /data/usenet/completed/books",
        )

        print("\n[OK] Arr SAB remote path mapping checks")
        self._check_arr_remote_mapping(
            "sonarr",
            8989,
            "v3",
            "/config/Downloads/complete",
            "/data/usenet/completed",
            "Sonarr has SAB legacy->local remote path mapping",
        )
        self._check_arr_remote_mapping(
            "radarr",
            7878,
            ...
            "/config/Downloads/complete",
            "/data/usenet/completed",
            "Radarr has SAB legacy->local remote path mapping",
        )
        self._check_arr_remote_mapping(
            "lidarr",
            8686,
            ...
            "/config/Downloads/complete",
            "/data/usenet/completed",
            "Lidarr has SAB legacy->local remote path mapping",
        )
        self._check_arr_remote_mapping(
            "readarr",
            8787,
            ...
            "/config/Downloads/complete",
            "/data/usenet/completed",
            "Readarr has SAB legacy->local remote path mapping",
        )
