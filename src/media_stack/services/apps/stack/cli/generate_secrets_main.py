...
Behavior mirrors bin/generate-secrets.sh while keeping shell wrappers thin.
"""
...
class GenerateSecretsCommand:
    """Encapsulates the generate-secrets CLI workflow."""
    ...
    def _safe_b64decode(self, value: str | None) -> str:
        if not value:
            return ""
        try:
    ...
    def _build_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--rotate-existing",
            action="store_true",
            default=self._env_truthy("ROTATE_EXISTING", False),
        )
        parser.add_argument(
            "--pass-length",
            type=int,
    ...
    def parse_config(self, argv: list[str] | None = None) -> GenerateSecretsConfig:
    ...
    def _get_secret_payload(
        ...
        if not isinstance(data, dict):
            return {}
        return {str(k): self._safe_b64decode(str(v)) for k, v in data.items()}
    ...
    def _write_output(
        self, path: Path, values: dict[str, str], namespace: str, secret_name: str
    ) -> None:
        path.parent.mkdir(parents=True, exist_ok=True)
        content = (
            "# Generated by bin/generate-secrets.sh\n"
    ...
    def run(self, cfg: GenerateSecretsConfig) -> int:
        kubectl = resolve_kubectl_binary()
        current = self._get_secret_payload(kubectl, cfg.namespace, cfg.secret_name)
        existed = bool(current)

        values = self.build_secret_values(
            current=current,
        ...
        self._apply_secret(kubectl, cfg.namespace, cfg.secret_name, values)
        self._write_output(cfg.output_file, values, cfg.namespace, cfg.secret_name)
...
_COMMAND = GenerateSecretsCommand()

_env_truthy = _COMMAND._env_truthy
_safe_b64decode = _COMMAND._safe_b64decode
_run = _COMMAND._run
_rand_secret = _COMMAND._rand_secret
_build_parser = _COMMAND._build_parser
parse_config = _COMMAND.parse_config
_get_secret_payload = _COMMAND._get_secret_payload
_apply_secret = _COMMAND._apply_secret
build_secret_values = _COMMAND.build_secret_values
_write_output = _COMMAND._write_output
run = _COMMAND.run
main = _COMMAND.main
