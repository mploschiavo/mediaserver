...
class PlaywrightScreenshotsMain:
    ...
    def build_arg_parser(self) -> argparse.ArgumentParser:
        parser = argparse.ArgumentParser(
            prog="bin/run-playwright-screenshots.sh",
    ...
    def main(self, argv: list[str] | None = None) -> int:
        ...
        if not node_ip:
            raise ConfigError(
                "Missing NODE_IP. Usage: bin/run-playwright-screenshots.sh <NODE_IP> [NAMESPACE] [OUT_DIR]"
            )
        if not namespace:
...
_instance = PlaywrightScreenshotsMain()
info = _instance.info
