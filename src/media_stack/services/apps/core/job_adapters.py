...
ADR-0012 redo: every helper is a plain instance method on one of
nine service-cohesion classes (``_HttpRequestFactory``,
``_LegacyArgsHelpers``, ``CoreActionAdapters``,
``IndexerJobAdapters``, ``BazarrEnsureJobAdapters``,
``ArrEnsureJobAdapters``, ``ServiceEnsureJobAdapters``,
``MediaIntegrityJobAdapters``, ``ApiKeyDiscoveryJobAdapters``).
Module-level singletons + assignment aliases preserve every
public ``ensure_*``/``action`` name AND every underscore-prefixed
helper that test-suite ``mock.patch(\"…job_adapters._<name>\")``
calls expect to intercept.
"""
...
# ----------------------------------------------------------------------
# 1. _HttpRequestFactory — redirect-safe HTTP request builder
# ----------------------------------------------------------------------


class _HttpRequestFactory:
    """Build HTTP-request callables that survive URL-base prefix
    redirects. Pulled out so every *arr-flavored adapter shares the
    same constructor."""

    def make_servarr_http_request(self) -> Callable[..., tuple]:
...
# ----------------------------------------------------------------------
# 2. _LegacyArgsHelpers — args/state/url helpers for legacy handlers
# ----------------------------------------------------------------------


class _LegacyArgsHelpers:
    """Shape-conversion helpers between ``JobContext`` and the legacy
    ``argparse.Namespace`` + state-stub the action handlers expect."""

    def default_args(self, ctx: JobContext) -> argparse.Namespace:
    ...
    def strip_api_key_from_url(self, url: str) -> str:
...
# ----------------------------------------------------------------------
# 3. CoreActionAdapters — six core ``action_*`` wrappers
# ----------------------------------------------------------------------


class CoreActionAdapters:
    """Job-framework wrappers around the original
    ``action_*`` handlers in
    ``media_stack.services.jobs.action_handlers``."""

    def envoy_config(self, ctx: JobContext) -> dict:
        """Regenerate Envoy routing config + restart envoy."""
        from media_stack.services.jobs.action_handlers import action_envoy_config
        action_envoy_config(_self_module()._default_args(ctx))
        return {"action": "envoy-config"}

    def ingress_config(self, ctx: JobContext) -> dict:
    ...
    def seed_runtime_overrides(self, ctx: JobContext) -> dict:
    ...
    def validate_credentials(self, ctx: JobContext) -> dict:
    ...
    def restart_apps(self, ctx: JobContext) -> dict:
        ...
        mod = _self_module()
        action_restart_apps(
            mod._default_args(ctx), mod._stub_state(),
            _load_handler_specs, _run_handler_specs,
        )
        return {"action": "restart-apps"}

    def post_setup(self, ctx: JobContext) -> dict:
    ...
    def run_legacy_pipeline(self, ctx: JobContext) -> dict:
        ...
        from media_stack.services.jobs.controller_runner import _build_runner
        runner, runtime_state = _build_runner(_self_module()._default_args(ctx))
        runner.run(runtime_state)
        return {"action": "run-legacy-pipeline"}


# ----------------------------------------------------------------------
# 4. IndexerJobAdapters — Prowlarr/indexer reconciliation jobs
# ----------------------------------------------------------------------


class IndexerJobAdapters:
    """Indexer discovery + Prowlarr reconciliation adapters."""

    def discover_indexers(self, ctx: JobContext) -> dict:
        ...
        try:
            ...
            from media_stack.services.jobs.controller_runner import _build_runner
            action_discover_indexers(_self_module()._default_args(ctx), _build_runner)
    ...
    def tag_indexers_for_apps(self, ctx: JobContext) -> dict:
        ...
        _http_request = _self_module()._make_servarr_http_request()
    ...
    def reset_prowlarr_app_mappings(self, ctx: JobContext) -> dict:
        ...
        try:
            ...
            try:
                for app in ("Sonarr", "Radarr", "Lidarr", "Readarr"):
                    ...
                    http = _self_module()._make_servarr_http_request()
    ...
    def push_indexers(self, ctx: JobContext) -> dict:
        """Trigger indexer-manager ApplicationIndexerSync."""
        from media_stack.services.jobs.action_handlers import action_push_indexers
        from media_stack.services.jobs.controller_runner import _build_runner
        action_push_indexers(_self_module()._default_args(ctx), _build_runner)
        return {"action": "push-indexers"}


# ----------------------------------------------------------------------
# 5. BazarrEnsureJobAdapters — Bazarr language profile + plugin config
# ----------------------------------------------------------------------


class BazarrEnsureJobAdapters:
    """Bazarr-side ensure-* jobs."""

    def ensure_bazarr_language_profile(self, ctx: JobContext) -> dict:
...
# ----------------------------------------------------------------------
# 6. ArrEnsureJobAdapters — *arr download client / seed series / notifier
# ----------------------------------------------------------------------


class ArrEnsureJobAdapters:
    """*arr-side ensure-* jobs (download client, notifier, seed series,
    runtime defaults, qBit categories)."""

    def ensure_qbittorrent_categories(self, ctx: JobContext) -> dict:
    ...
    def ensure_arr_download_client(self, ctx: JobContext) -> dict:
    ...
    def ensure_sonarr_seed_series(self, ctx: JobContext) -> dict:
    ...
    def ensure_arr_jellyfin_notifier(self, ctx: JobContext) -> dict:
    ...
    def apply_arr_runtime_defaults(self, ctx: JobContext) -> dict:
...
# ----------------------------------------------------------------------
# 7. ServiceEnsureJobAdapters — Jellyfin libraries + Jellyseerr OIDC
# ----------------------------------------------------------------------


class ServiceEnsureJobAdapters:
    """Service-side ensure-* jobs (Jellyfin libraries, Jellyseerr OIDC)."""

    def ensure_jellyfin_libraries(self, ctx: JobContext) -> dict:
        ...
        try:
            req = _ur.Request(
                _self_module()._strip_api_key_from_url(f"{base}/Library/VirtualFolders"),
                headers={"Accept": "application/json",
                         "X-Emby-Token": jf_key},
            )
            with _ur.urlopen(req, timeout=_HTTP_GET_TIMEOUT_S) as r:
    ...
    def ensure_jellyseerr_oidc(self, ctx: JobContext) -> dict:
...
# ----------------------------------------------------------------------
# 8. MediaIntegrityJobAdapters — hygiene / mass-search / recovery
# ----------------------------------------------------------------------


class MediaIntegrityJobAdapters:
    """Library-integrity adapters: hygiene, mass-search, recovery,
    completed-downloads scan."""

    def run_media_hygiene(self, ctx: JobContext) -> dict:
    ...
    def mass_search_qbit_active_count(self, ctx: JobContext) -> int | None:
    ...
    def mass_search_throttled(self, ctx: JobContext) -> dict:
        ...
        qbit_active = _self_module()._mass_search_qbit_active_count(ctx)
    ...
    def qbit_completed_torrents(self, ctx: JobContext) -> list[dict]:
    ...
    def recover_stuck_imports(self, ctx: JobContext) -> dict:
        ...
        qbit_done = _self_module()._qbit_completed_torrents(ctx)
    ...
    def scan_completed_downloads(self, ctx: JobContext) -> dict:
...
# ----------------------------------------------------------------------
# 9. ApiKeyDiscoveryJobAdapters — discover-api-keys + helpers
# ----------------------------------------------------------------------
...
class ApiKeyDiscoveryJobAdapters:
    """API-key discovery adapter + on-disk harvest + secret-patch persist."""

    def discover_api_keys(self, ctx: JobContext) -> dict:
        ...
        """
        mod = _self_module()
        args = mod._default_args(ctx)
        state = mod._stub_state()
        skipped: list[str] = []
        ...
        discovered, file_skipped = mod._harvest_keys_from_disk(args.config_root)
        ...
        persist_result = mod._persist_preflight_keys_to_secret_safe(
            state, discovered,
        )
    ...
    def harvest_keys_from_disk(self, config_root: str) -> tuple[dict[str, str], list[str]]:
    ...
    def persist_preflight_keys_to_secret_safe(
        self, state: object, discovered: dict[str, str],
    ) -> dict:
...
# ----------------------------------------------------------------------
# Module-level singletons + aliases (ADR-0012 Phase E pattern)
# ----------------------------------------------------------------------
#
# Every public adapter name AND every underscore-prefixed helper that
# tests/external callers patch via
# ``mock.patch("media_stack.services.apps.core.job_adapters.<name>")``
...
_CORE_ACTION_ADAPTERS = CoreActionAdapters()
_INDEXER_JOB_ADAPTERS = IndexerJobAdapters()
_BAZARR_ENSURE_JOB_ADAPTERS = BazarrEnsureJobAdapters()
_ARR_ENSURE_JOB_ADAPTERS = ArrEnsureJobAdapters()
_SERVICE_ENSURE_JOB_ADAPTERS = ServiceEnsureJobAdapters()
_MEDIA_INTEGRITY_JOB_ADAPTERS = MediaIntegrityJobAdapters()
...
# --- _LegacyArgsHelpers ------------------------------------------------
_default_args = _LEGACY_ARGS_HELPERS.default_args
_strip_api_key_from_url = _LEGACY_ARGS_HELPERS.strip_api_key_from_url
_stub_state = _LEGACY_ARGS_HELPERS.stub_state
_detect_platform = _LEGACY_ARGS_HELPERS.detect_platform

# --- CoreActionAdapters ------------------------------------------------
envoy_config = _CORE_ACTION_ADAPTERS.envoy_config
ingress_config = _CORE_ACTION_ADAPTERS.ingress_config
seed_runtime_overrides = _CORE_ACTION_ADAPTERS.seed_runtime_overrides
validate_credentials = _CORE_ACTION_ADAPTERS.validate_credentials
restart_apps = _CORE_ACTION_ADAPTERS.restart_apps
post_setup = _CORE_ACTION_ADAPTERS.post_setup
run_legacy_pipeline = _CORE_ACTION_ADAPTERS.run_legacy_pipeline

# --- IndexerJobAdapters ------------------------------------------------
discover_indexers = _INDEXER_JOB_ADAPTERS.discover_indexers
tag_indexers_for_apps = _INDEXER_JOB_ADAPTERS.tag_indexers_for_apps
reset_prowlarr_app_mappings = _INDEXER_JOB_ADAPTERS.reset_prowlarr_app_mappings
push_indexers = _INDEXER_JOB_ADAPTERS.push_indexers

# --- BazarrEnsureJobAdapters -------------------------------------------
ensure_bazarr_language_profile = _BAZARR_ENSURE_JOB_ADAPTERS.ensure_bazarr_language_profile

# --- ArrEnsureJobAdapters ----------------------------------------------
ensure_qbittorrent_categories = _ARR_ENSURE_JOB_ADAPTERS.ensure_qbittorrent_categories
ensure_arr_download_client = _ARR_ENSURE_JOB_ADAPTERS.ensure_arr_download_client
ensure_sonarr_seed_series = _ARR_ENSURE_JOB_ADAPTERS.ensure_sonarr_seed_series
ensure_arr_jellyfin_notifier = _ARR_ENSURE_JOB_ADAPTERS.ensure_arr_jellyfin_notifier
apply_arr_runtime_defaults = _ARR_ENSURE_JOB_ADAPTERS.apply_arr_runtime_defaults

# --- ServiceEnsureJobAdapters ------------------------------------------
ensure_jellyfin_libraries = _SERVICE_ENSURE_JOB_ADAPTERS.ensure_jellyfin_libraries
ensure_jellyseerr_oidc = _SERVICE_ENSURE_JOB_ADAPTERS.ensure_jellyseerr_oidc

# --- MediaIntegrityJobAdapters -----------------------------------------
run_media_hygiene = _MEDIA_INTEGRITY_JOB_ADAPTERS.run_media_hygiene
_mass_search_qbit_active_count = _MEDIA_INTEGRITY_JOB_ADAPTERS.mass_search_qbit_active_count
mass_search_throttled = _MEDIA_INTEGRITY_JOB_ADAPTERS.mass_search_throttled
_qbit_completed_torrents = _MEDIA_INTEGRITY_JOB_ADAPTERS.qbit_completed_torrents
recover_stuck_imports = _MEDIA_INTEGRITY_JOB_ADAPTERS.recover_stuck_imports
scan_completed_downloads = _MEDIA_INTEGRITY_JOB_ADAPTERS.scan_completed_downloads

# --- ApiKeyDiscoveryJobAdapters ----------------------------------------
discover_api_keys = _API_KEY_DISCOVERY_JOB_ADAPTERS.discover_api_keys
_harvest_keys_from_disk = _API_KEY_DISCOVERY_JOB_ADAPTERS.harvest_keys_from_disk
_persist_preflight_keys_to_secret_safe = (
    _API_KEY_DISCOVERY_JOB_ADAPTERS.persist_preflight_keys_to_secret_safe
)
