...
def parse_oidc_config(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    for parser in (_parse_google, _parse_auth0, _parse_okta,
                   _parse_microsoft, _parse_keycloak, _parse_github,
                   _parse_generic):
        result = parser(raw)
        if result:
            result["raw"] = raw
    ...
    return {"provider": "unknown", "error": "Could not detect provider from JSON structure", "raw": raw}


def _parse_google(raw: dict[str, Any]) -> dict[str, Any] | None:
...
def _parse_auth0(raw: dict[str, Any]) -> dict[str, Any] | None:
...
def _parse_okta(raw: dict[str, Any]) -> dict[str, Any] | None:
...
def _parse_microsoft(raw: dict[str, Any]) -> dict[str, Any] | None:
...
def _parse_keycloak(raw: dict[str, Any]) -> dict[str, Any] | None:
...
def _parse_github(raw: dict[str, Any]) -> dict[str, Any] | None:
...
def _parse_generic(raw: dict[str, Any]) -> dict[str, Any] | None:
