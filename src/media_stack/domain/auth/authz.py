...
def _extract_actor(kwargs: dict, default: Any = inspect.Parameter.empty) -> Actor:
...
def _actor_default_from_signature(fn: Callable[..., Any]) -> Any:
...
def requires_authenticated(fn: F) -> F:
    """Caller must be a logged-in human or a system actor."""

    default = _actor_default_from_signature(fn)

    @wraps(fn)
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        actor = _extract_actor(kwargs, default=default)
...
def requires_self_or_admin(param: str) -> Callable[[F], F]:
...
def forbidden_for_impersonation(fn: F) -> F:
