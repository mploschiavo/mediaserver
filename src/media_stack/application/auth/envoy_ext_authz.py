...
from media_stack.domain.auth.gateway_policy import ExtAuthzConfig, GatewayAuthPolicy
...
class EnvoyExtAuthzHelpers:
    """Builders for Envoy ext_authz filter, cluster, and per-route config.

    Per ADR-0012: plain instance methods, no @staticmethod. Module-level
    aliases below preserve the public function-style API for callers and
    tests.
    """

    def _prefix_with_rd(self, raw_prefix: str, auth_portal_url: str) -> str:
    ...
    def build_ext_authz_filter(
        ...
        """
        path_prefix = self._prefix_with_rd(ext_authz.path_prefix, auth_portal_url)
    ...
    def build_ext_authz_cluster(self, ext_authz: ExtAuthzConfig) -> dict[str, Any]:
    ...
    def route_ext_authz_disabled_config(self) -> dict[str, Any]:
    ...
    def inject_ext_authz_into_payload(
        ...
        ext_authz_filter = self.build_ext_authz_filter(policy.ext_authz, auth_portal_url)
        ...
        auth_cluster = self.build_ext_authz_cluster(policy.ext_authz)
    ...
    def apply_per_route_auth_policy(
        ...
        if svc_policy in ("native", "public"):
            ...
            per_filter.update(self.route_ext_authz_disabled_config())


_INSTANCE = EnvoyExtAuthzHelpers()

# Module-level aliases preserve the public function-style API.
# Tests import these names directly; the wildcard re-export shim at
# media_stack.core.auth.envoy_ext_authz picks them up via vars(_impl).
_prefix_with_rd = _INSTANCE._prefix_with_rd
build_ext_authz_filter = _INSTANCE.build_ext_authz_filter
build_ext_authz_cluster = _INSTANCE.build_ext_authz_cluster
route_ext_authz_disabled_config = _INSTANCE.route_ext_authz_disabled_config
inject_ext_authz_into_payload = _INSTANCE.inject_ext_authz_into_payload
apply_per_route_auth_policy = _INSTANCE.apply_per_route_auth_policy
