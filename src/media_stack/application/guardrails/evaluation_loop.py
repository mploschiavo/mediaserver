...
def _dispatch_actions(
    ...
    *,
    ...
    Mutates each Action in place: ``ok`` is flipped to ``True`` when
    the dispatch succeeded, and ``detail`` is updated with the
    post-dispatch summary so the audit log surfaces the actual
    outcome rather than the rule's pre-dispatch intent.
    ...
    if lockdown_service is None:
        for action in actions:
            if action.action in (
                ...
                _log.debug(
                    "guardrails: %s not dispatched — no lockdown service "
    ...
    for action in actions:
        if action.action == _ACTION_LOCKDOWN_ENGAGE:
            try:
                trigger_label = _engage_trigger_label(state)
                ...
                _log.warning(
                    "guardrails: lockdown engage dispatch failed: %s", exc,
                )
                action.ok = False
            ...
            try:
                result = lockdown_service.release(by="auto:disk-recovered")
            except (OSError, RuntimeError, ValueError) as exc:
                _log.warning(
...
def _engage_trigger_label(state: Mapping[str, Any]) -> str:
...
def tick(
    *,
    ...
    ``lockdown_service`` is the ``DownloadLockdownService`` instance
    the dispatcher hands ``lockdown_engage`` / ``lockdown_release``
    actions to. Production wires it in via the auto-heal loop's
    bootstrap; unit tests inject a fake or pass ``None`` to skip
    dispatch entirely."""
