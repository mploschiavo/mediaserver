...
Per ADR-0012, all helpers are plain instance methods on
``StateCollector``. Module-level aliases below preserve the
import-as-function call sites; ``collect_state`` dispatches to the
``_indexer_429_*`` snapshot helpers through ``sys.modules[__name__]``
so any future ``mock.patch`` of those names wins.
"""
...
# window, etc.
#
# The window list and its lock are module-scope (not bound to the
# StateCollector instance) so that test reset hooks and any
# concurrent updater observe the same state through the module
# globals.
...
class StateCollector:
    """Builds the per-tick guardrails state snapshot.

    Per ADR-0012, every public surface name on this module routes
    through one of these methods via the module-level aliases
    declared at the bottom of the file.
    """

    def record_indexer_429(
        self,
        indexer_id: str,
        *,
        now: float | None = None,
    ...
    def collect_state(
        ...
        *,
        ...
        # when telemetry isn't wired. Snapshot helpers route through
        # the module so test ``mock.patch`` of either name wins.
        state["bandwidth"] = {
            "upload_gb_today": 0.0,
            "concurrent_downloads": 0,
            "indexer_429s": mod._indexer_429_snapshot(now=ts),
        }
        state["external_api"] = {
            "opensubtitles_used": 0,
            "tmdb_calls_today": 0,
            "indexer_429s": mod._indexer_429_hour(now=ts),
        }
...
_INSTANCE = StateCollector()

# Module-level aliases for every public + underscore name (ADR-0012).
# Internal cross-method calls inside ``collect_state`` route through
# ``sys.modules[__name__]`` so ``mock.patch`` on these names wins.
record_indexer_429 = _INSTANCE.record_indexer_429
_indexer_429_snapshot = _INSTANCE._indexer_429_snapshot
_indexer_429_hour = _INSTANCE._indexer_429_hour
collect_state = _INSTANCE.collect_state
