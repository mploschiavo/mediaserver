...
class MaintainerrPolicyService:
    ...
    def _repo_bootstrap_defaults_dir() -> Path:
        # NOTE: this module now lives at
        # ``src/media_stack/application/maintainerr/policy_service.py``,
        # so the package's ``contracts/`` directory sits two parents
        # up (maintainerr → application → media_stack/contracts). The
        # legacy ``services/apps/maintainerr/policy_service.py`` used
        # ``parents[3]`` to land at the same ``media_stack/contracts``
        # directory.
