...
ADR-0012 layout:
    Plain instance methods on ``JellyfinEnsureApiKeyHelpers`` — module-level
    aliases at the bottom expose the historical function-style names so
    the job framework's name-based handler lookup and existing tests'
    ``mock.patch("…ensure_api_key._jellyfin_reachable", …)`` keep working
    without changes. Internal cross-method calls dispatch through
...
class JellyfinEnsureApiKeyHelpers:
    """ADR-0012: class-based handlers/services with constructor-injected
    deps (none here — module logger is fine for a bootstrap helper).

    Internal cross-method calls dispatch through ``sys.modules[__name__]``
    so unit tests that ``mock.patch`` the module-level alias names
    (``_jellyfin_reachable``, ``_persist_to_secret_if_possible``,
    ``_bust_runtime_keys_cache``) are observed by ``ensure_jellyfin_api_key``.
    """

    def ensure_jellyfin_api_key(self, _ctx: JobContext) -> dict[str, Any]:
        ...
        if not module._jellyfin_reachable(jellyfin_url):
            return {
        ...
        persist_summary = module._persist_to_secret_if_possible(
            {"JELLYFIN_API_KEY": minted, **({"JELLYFIN_USER_ID": user_id} if user_id else {})},
        )
        module._bust_runtime_keys_cache()
    ...
    def jellyfin_reachable(self, jellyfin_url: str, timeout_seconds: int = 5) -> bool:
    ...
    def persist_to_secret_if_possible(self, payload: dict[str, str]) -> str:
    ...
    def bust_runtime_keys_cache(self) -> None:
...
_INSTANCE = JellyfinEnsureApiKeyHelpers()

# Module-level aliases preserve the historical function-style API.
# The job framework's name-based handler lookup imports
# ``ensure_jellyfin_api_key`` from this module; tests ``mock.patch`` the
# underscore helpers as module attributes. The ``ensure_jellyfin_api_key``
# method dispatches through ``sys.modules[__name__]`` so those patches
# take effect.
ensure_jellyfin_api_key = _INSTANCE.ensure_jellyfin_api_key
_jellyfin_reachable = _INSTANCE.jellyfin_reachable
_persist_to_secret_if_possible = _INSTANCE.persist_to_secret_if_possible
_bust_runtime_keys_cache = _INSTANCE.bust_runtime_keys_cache
