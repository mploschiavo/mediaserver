...
ADR-0012: top-level FunctionDef count is held at 0 — every helper
is an instance method on ``ArrRuntimeDefaults`` and exposed as a
module-level alias bound to the singleton ``_INSTANCE``."""
...
class ArrRuntimeDefaults:
    """Per-arr runtime-defaults helpers (ADR-0012 OO container).

    Plain instance methods, NO ``@staticmethod``. Module-level
    aliases below bind every public name to the singleton
    ``_INSTANCE``."""
    ...
    def apply_arr_runtime_defaults(
        ...
        *,
        ...
        Helper calls go through ``sys.modules[__name__]`` so any
        ``mock.patch`` of the module-level alias takes effect even
        though the dispatcher is itself a method on this class."""
        import sys
        ...
        if "radarr" in by_impl and "radarr" in app_keys:
            try:
                n = _mod.patch_radarr_language_any(
        ...
        for impl, api_ver in (
            ...
            if impl not in by_impl or impl not in app_keys:
                continue
            try:
                n = _mod.patch_arr_import_lists_auto(
                    arr_url=service_url(impl),
                    api_ver=api_ver,
                    api_key=app_keys[impl],
                    http_request=http_request,
        ...
        if "lidarr" in by_impl and "lidarr" in app_keys:
            try:
                n = _mod.patch_lidarr_quality_sizes(
        ...
        if "readarr" in by_impl and "readarr" in app_keys:
            try:
                n = _mod.patch_readarr_allow_unknown_text(
...
_INSTANCE = ArrRuntimeDefaults()

# ADR-0012: module-level aliases for every public helper. Tests +
# external callers continue to import these names verbatim; the
# dispatcher (``apply_arr_runtime_defaults``) re-resolves through
# ``sys.modules[__name__]`` so ``mock.patch`` of a module-level
# alias correctly intercepts dispatcher-internal calls.
patch_radarr_language_any = _INSTANCE.patch_radarr_language_any
patch_lidarr_quality_sizes = _INSTANCE.patch_lidarr_quality_sizes
patch_readarr_allow_unknown_text = _INSTANCE.patch_readarr_allow_unknown_text
patch_arr_usenet_enabled = _INSTANCE.patch_arr_usenet_enabled
patch_arr_import_lists_auto = _INSTANCE.patch_arr_import_lists_auto
apply_arr_runtime_defaults = _INSTANCE.apply_arr_runtime_defaults
