...
class LifecycleResolver:
    ...
    def _container_access_for(self, service_id: str) -> Any | None:
        ...
        Tries compose first (docker-py container lookup), then k8s
        (kubectl + deployment selector). Returns the first impl that
        successfully resolves to a reachable workload. Returns
        ``None`` when neither platform is reachable — the lifecycle
        method treats that as "no rotation possible, surface the
        error honestly" rather than silently dropping work.
        ADR-0013 Phase 3 design principle 5 (no silent skips).
        """
        try:
            compose = self._compose_container_access(service_id)
        ...
        try:
            return self._k8s_container_access(service_id)
        except Exception as exc:  # noqa: BLE001
            logger.debug(
                "container_access k8s lookup failed for %s: %s",
                service_id, exc,
            )
            return None

    def _compose_container_access(self, service_id: str) -> Any | None:
        """Resolve a compose ``ContainerAccess`` for ``service_id``.

        Looks up the docker-py container by name; returns ``None``
        if docker is unreachable or the container doesn't exist
        (compose isn't the active platform here).
        """
        try:
    ...
    def _k8s_container_access(self, service_id: str) -> Any | None:
        """Resolve a k8s ``ContainerAccess`` for ``service_id``.

        Uses ``kubectl`` against the ``deploy/<service_id>`` selector
        in the env-configured namespace. Returns ``None`` if kubectl
        isn't on PATH, the namespace can't be resolved, or the
        ...
        try:
            tokens = kube_cmd()
        except Exception:  # noqa: BLE001 — resolver may raise on missing kubectl
            return None
        if not tokens or not shutil.which(tokens[0]):
            return None
        namespace = (os.environ.get("K8S_NAMESPACE", "") or "media-stack").strip()
        selector = f"deploy/{service_id}"
        # Existence probe: ``kubectl get deploy <name>`` returns 0 if
        # found, non-zero otherwise. Cheap (one API call), avoids
        # spinning up a non-functional ContainerAccess.
        try:
            proc = subprocess.run(
                [*tokens, "-n", namespace, "get", selector,
                 "--no-headers", "-o", "name"],
        ...
        if proc.returncode != 0 or not (proc.stdout or "").strip():
            return None
        from media_stack.infrastructure.platforms.k8s.container_access import (
            K8sContainerAccess,
        )
        return K8sContainerAccess(
            kube_cmd=tokens,
            namespace=namespace,
            selector=selector,
        )
