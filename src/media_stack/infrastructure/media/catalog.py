...
def _candidate_yaml_locations() -> list[Path]:
...
_cache: dict[str, MediaType] | None = None


def _parse_media_types_yaml(path: Path) -> dict[str, MediaType] | None:
    ...
    if not isinstance(block, dict):
        ...
        return None
    return _deserialize_catalog_block(block)


def _deserialize_catalog_block(block: dict[str, Any]) -> dict[str, MediaType]:
...
def load_media_types() -> dict[str, MediaType]:
    ...
    if _cache is not None:
        return _cache
    for candidate in _candidate_yaml_locations():
        if not candidate.is_file():
            continue
        catalog = _parse_media_types_yaml(candidate)
...
def media_type(name: str) -> MediaType | None:
    ...
    return load_media_types().get(name)
