...
4. If your PR changes a behavior covered by [`contracts/promises/cross_cutting.yaml`](contracts/promises/cross_cutting.yaml) or a per-service `contracts/services/<svc>.yaml` `plugin.promises:` block (ADR-0006 per-service registries), update the promise. If you add a new OTB guarantee, add a new promise and re-run `media-stack-render-promises`.
