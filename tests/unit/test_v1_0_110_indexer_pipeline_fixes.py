...
class RibbonBarFiltersDisabledServices(unittest.TestCase):
    """Ribbon bar (``buildQuickLinks``) used to render every
    registry entry. Operators with a 'standard' install (no
    Plex, no Authentik) saw dead links to services that aren't
    even running."""

    def test_buildquicklinks_skips_disabled_and_no_port(self) -> None:
        text = (ROOT / "src/media_stack/api/dashboard.html").read_text(encoding="utf-8")
        # Find the function body.
        idx = text.find("function buildQuickLinks(){")
        self.assertGreater(idx, 0)
        body = text[idx:idx + 600]
        self.assertIn("if(!s.port)continue", body,
                      "ribbon-bar no longer skips zero-port "
                      "(placeholder) services — dead links return.")
        self.assertIn("SVC_HEALTH[s.id]==='disabled'", body,
                      "ribbon-bar no longer skips disabled "
                      "services — dead links return.")

    def test_quicklinks_rebuild_after_health_probe(self) -> None:
        """probeServices() updates SVC_HEALTH; ribbon must
        re-render so newly-disabled services drop out."""
        text = (ROOT / "src/media_stack/api/dashboard.html").read_text(encoding="utf-8")
        # buildQuickLinks must be called inside probeServices() body.
        probe_start = text.index("async function probeServices(){")
        probe_end = text.index("function renderHealthStories(", probe_start)
        body = text[probe_start:probe_end]
        self.assertIn("buildQuickLinks()", body,
                      "probeServices no longer triggers ribbon "
                      "rebuild — disabled services persist in the "
                      "ribbon until next page reload.")
