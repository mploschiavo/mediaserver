"""Pin the per-service-registry shape of the Maintainerr family.

The split moved the single Maintainerr-family promise out of the
monolithic ``contracts/promises/promises.yaml`` into
``contracts/services/maintainerr.yaml::plugin.promises:``. This
ratchet asserts the post-migration shape so a future contract
edit can't silently undo it.

Sections:

  * MaintainerrPromisesLoadFromServiceYaml — the loader's
    source-path map points the Maintainerr promise at the
    maintainerr service yaml.
  * LegacyMonolithDoesNotContainMaintainerrPromises — the id
    is GONE from the cross-cutting / legacy promises.yaml.
  * BootstrapBlockingResolvesToLoaderDefault — the promise does
    not carry an explicit ``bootstrap_blocking:`` line, so the
    loader default (True) applies. Pinning the resolved value
...
# Mirrors the equivalent servarr/jellyfin fixture shape; only one
# Maintainerr promise migrates.
_PROMISE_SOURCE_MAP: dict[str, Path] = {
    "maintainerr-rules-linked-to-arr": Path("services") / "maintainerr.yaml",
}
...
class MaintainerrPromisesLoadFromServiceYaml(unittest.TestCase):
    """The Maintainerr promise has its source path pointing at
    ``contracts/services/maintainerr.yaml``, NOT at the
    ...
    def test_maintainerr_rules_linked_to_arr_loads_from_maintainerr_yaml(
        self,
    ) -> None:
        self.assert_loaded_from_service_yaml(
            "maintainerr-rules-linked-to-arr",
            _PROMISE_SOURCE_MAP["maintainerr-rules-linked-to-arr"],
        )


class LegacyMonolithDoesNotContainMaintainerrPromises(unittest.TestCase):
    """The move is loss-free, not duplicate-friendly. The
    Maintainerr id must NOT appear in the monolithic
    ...
    def test_no_maintainerr_id_in_legacy_yaml(self) -> None:
...
class BootstrapBlockingResolvesToLoaderDefault(unittest.TestCase):
    """The Maintainerr promise does not carry an explicit
    ...
    loader default would flip the promise silently — pinning the
    resolved value here makes that surface as a test failure,
    ...
    def test_each_maintainerr_promise_resolves_as_blocking(self) -> None:
    ...
    def test_no_unexpected_cross_family_dependencies(self) -> None:
        # The Maintainerr promise had no ``depends_on`` in
