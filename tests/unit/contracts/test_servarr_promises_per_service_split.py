"""Pin the per-service-registry shape of the Servarr family.

The split moved the eleven Servarr-family promises out of the
monolithic ``contracts/promises/promises.yaml`` into their
respective ``contracts/services/<svc>.yaml::plugin.promises:``
blocks (Sonarr x5, Radarr x5, Lidarr x1). This ratchet asserts
the post-migration shape so a future contract edit can't
silently undo it.

Sections:

  * ServarrPromisesLoadFromServiceYaml — the loader's source-path
    map points each Servarr promise at the right service yaml.
  * LegacyMonolithDoesNotContainServarrPromises — the eleven ids
    are GONE from the cross-cutting / legacy promises.yaml.
  * BootstrapBlockingResolvesToLoaderDefault — none of these
    eleven promises carry an explicit ``bootstrap_blocking:``
    line, so the loader default (True) applies. Pinning the
    resolved value catches a future loader-default flip.
...
# Mirrors the equivalent jellyfin fixture shape but expanded across three
# service yaml files.
_PROMISE_SOURCE_MAP: dict[str, Path] = {
    # Sonarr
    "sonarr-jellyfin-notifier": Path("services") / "sonarr.yaml",
    "sonarr-has-indexers": Path("services") / "sonarr.yaml",
    "sonarr-has-series": Path("services") / "sonarr.yaml",
    "sonarr-download-client": Path("services") / "sonarr.yaml",
    "sonarr-quality-profiles": Path("services") / "sonarr.yaml",
    # Radarr
    "radarr-jellyfin-notifier": Path("services") / "radarr.yaml",
    "radarr-has-indexers": Path("services") / "radarr.yaml",
    "radarr-import-lists-auto": Path("services") / "radarr.yaml",
    "radarr-download-client": Path("services") / "radarr.yaml",
    "radarr-quality-profiles": Path("services") / "radarr.yaml",
    # Lidarr
    "lidarr-jellyfin-notifier": Path("services") / "lidarr.yaml",
}
...
class ServarrPromisesLoadFromServiceYaml(unittest.TestCase):
    """Each of the eleven Servarr promises has its source path
    pointing at the corresponding ``contracts/services/<svc>.yaml``,
    NOT at the cross-cutting registry."""
    ...
    def test_sonarr_jellyfin_notifier_loads_from_sonarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "sonarr-jellyfin-notifier",
            _PROMISE_SOURCE_MAP["sonarr-jellyfin-notifier"],
        )

    def test_sonarr_has_indexers_loads_from_sonarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "sonarr-has-indexers",
            _PROMISE_SOURCE_MAP["sonarr-has-indexers"],
        )

    def test_sonarr_has_series_loads_from_sonarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "sonarr-has-series",
            _PROMISE_SOURCE_MAP["sonarr-has-series"],
        )

    def test_sonarr_download_client_loads_from_sonarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "sonarr-download-client",
            _PROMISE_SOURCE_MAP["sonarr-download-client"],
        )

    def test_sonarr_quality_profiles_loads_from_sonarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "sonarr-quality-profiles",
            _PROMISE_SOURCE_MAP["sonarr-quality-profiles"],
        )

    def test_radarr_jellyfin_notifier_loads_from_radarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "radarr-jellyfin-notifier",
            _PROMISE_SOURCE_MAP["radarr-jellyfin-notifier"],
        )

    def test_radarr_has_indexers_loads_from_radarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "radarr-has-indexers",
            _PROMISE_SOURCE_MAP["radarr-has-indexers"],
        )

    def test_radarr_import_lists_auto_loads_from_radarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "radarr-import-lists-auto",
            _PROMISE_SOURCE_MAP["radarr-import-lists-auto"],
        )

    def test_radarr_download_client_loads_from_radarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "radarr-download-client",
            _PROMISE_SOURCE_MAP["radarr-download-client"],
        )

    def test_radarr_quality_profiles_loads_from_radarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "radarr-quality-profiles",
            _PROMISE_SOURCE_MAP["radarr-quality-profiles"],
        )

    def test_lidarr_jellyfin_notifier_loads_from_lidarr_yaml(self) -> None:
        self.assert_loaded_from_service_yaml(
            "lidarr-jellyfin-notifier",
            _PROMISE_SOURCE_MAP["lidarr-jellyfin-notifier"],
        )


class LegacyMonolithDoesNotContainServarrPromises(unittest.TestCase):
    """The move is loss-free, not duplicate-friendly. The
    eleven Servarr ids must NOT appear in the monolithic
    ...
    def test_no_servarr_id_in_legacy_yaml(self) -> None:
        for pid in self._MIGRATED_IDS:
            self.assertNotIn(
                f"id: {pid}", self.text,
                f"{pid!r} reappeared in promises.yaml — the split "
                f"moved it into services/<svc>.yaml. Either "
...
class BootstrapBlockingResolvesToLoaderDefault(unittest.TestCase):
    """None of the eleven Servarr promises carry an explicit
    ...
    loader default would flip every Servarr promise silently —
    ...
    def test_each_servarr_promise_resolves_as_blocking(self) -> None:
    ...
    def test_no_unexpected_cross_family_dependencies(self) -> None:
        # None of the migrated Servarr promises had ``depends_on``
...
class RegistryCountUnchanged(unittest.TestCase):
    ...
    _EXPECTED_TOTAL = 59  # +5 ADR-0005 Phase 5c.1 wide ``*-api-key-discoverable``
