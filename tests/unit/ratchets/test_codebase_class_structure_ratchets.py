...
METHODS_OVER_50_LINES_RATCHET = 344  # 342 → 344 — ADR-0005 Phase 5c.4 closure: ``_run_serve`` grows ``_action_loop`` (the main retry/auto-heal loop, +50 lines) + ``_run_one_action`` watchdog wrapper. Same total line budget as the deleted subprocess loop, redistributed across two named functions.  # 340 → 333 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed several long handler methods. Tightened to new floor.
