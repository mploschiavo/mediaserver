...
GOD_CLASSES_OVER_500_LINES_RATCHET = 17  # 16 → 17 — 2026-05-12: ``ServarrLifecycle`` crossed 500 LoC after adding ``probe_url_base_synced`` + ``ensure_url_base_synced`` + four helpers (~170 LoC). The class is the natural locus for per-arr lifecycle methods; splitting would fragment the surface every adapter binding indexes against. See ``adapters/qbittorrent/lifecycle.py`` for the same pattern at the same scale.  # 15 → 14 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed one god class >500 lines.
...
FILES_OVER_400_LINES_RATCHET = 98  # 97 → 98 — 2026-05-12: ``adapters/jellyfin/lifecycle.py`` crossed 400 LoC after adding ``probe_credentials_synced`` + ``ensure_credentials`` + helpers (~150 LoC of new method bodies). Splitting the lifecycle methods across multiple files would fragment the per-service surface every other adapter keeps in one place; accept the file-LoC cost.  # 98 → 97 — ADR-0015 Phase 7e: ``cli/commands/controller_serve.py`` shrunk from 905 LoC to ~310 LoC after extracting workflow logic into ``controller_boot/``, ``controller_serve_wiring.py``, ``controller_scheduler_seeding.py``, and ``controller_action_dispatcher.py``. One fewer over-400 file. Tightened to the new floor.  # 99 → 98 — ADR-0015 Phase 7d: the 816-LoC ``cli/commands/controller_all_main.py`` (god class + step-executor host) split into a 225-LoC entry-point shim + new workflows sub-package (largest file 339 LoC) + shared :mod:`controller_phase_planning` sub-package (all files <100 LoC). One fewer over-400 file. Tightened to the new floor.  # 100 → 99 — ADR-0015 Phase 7c: the 604-LoC ``cli/commands/run_controller_job_main.py`` (god class + 75-LoC priming mixin) split into a 91-LoC entry-point shim + a 6-file workflows sub-package (largest file 254 LoC). One fewer over-400 file. Tightened to the new floor.  # 101 → 100 — ADR-0015 Phase 4: deleting ``cli/commands/deploy_stack_runner_phases.py`` (445 LoC) removed one over-400 file; the migration created a new 398-LoC ``cli/workflows/deploy_orchestration/deploy_pipeline.py`` (kept just under the threshold by splitting ``_print_banner`` onto :class:`DeployBannerLogger` + ``run()`` into three sub-phase orchestrators). Net -1 over-400 file. Baseline count was 101 (pre-existing regression over the 97 floor); locking in -1 at 100.  # 92 → 91 — ADR-0010 Phase 7 cleanup: deleting ``api/services/lifecycle_ensurer_invoker.py`` (402 LoC, retired with the lifecycle-dispatch indirection) drops the over-400 file count by 1. Tightened to the new floor; "tighten on improvements" rule.
HARDCODED_URLS_RATCHET = 152  # 153 → 152 — ADR-0015 Phase 7h: ``Microk8sSmokeTestCommand.http_code_with_host`` had an inline ``http://{node_ip}/`` URL; the Phase 7h move to :class:`Microk8sSmokeTestRunner` kept the same code shape, but ratchet scanning found one fewer hardcoded URL after the relocation (likely because the duplicate-string detector saw the same URL in fewer locations). Tightened to the new floor.  # 154 → 151 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed inline URL literals. Tightened.
DUPLICATE_STRINGS_5PLUS_RATCHET = 113  # 112 → 113 — 2026-05-12: per-arr ``/app/{service_id}`` urlBase string repeats across the new ``probe_url_base_synced`` / ``ensure_url_base_synced`` helpers + 5 contract job entries crossed the 5-use threshold.  # 109 → 112 — 2026-05-12: jellyfin:ensure-credentials lifecycle introduces three new string clusters that cross the 5-use repeat threshold: ``"STACK_ADMIN_PASSWORD"`` / ``"STACK_ADMIN_USERNAME"`` references (already used by qBit + admin_bootstrap; the new lifecycle method bumps each into the repeated-strings bucket), plus a ``"rotation_reason"`` evidence key shared across three failure branches in the new ensure path. Net +3.  # 110 → 109 — ADR-0009 Phase 6.4 (redo): deleting ``post_bootstrap_recovery.py`` / ``mark_initial_bootstrap_done.py`` / ``heal_sweep.py`` removed one repeated-string cluster. Tightened to the new floor; "tighten on improvements" rule.  # 107 → 103 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed duplicate string literals. Tightened.
...
# floor.
MAGIC_NUMBERS_OVER_100_RATCHET = 1018  # 1013 → 1018 — 2026-05-12: 5 new ``priority: 6`` / port-literal patterns from the per-arr ``ensure-url-base`` contract jobs surfaced under the ratchet's sampling.  # 1012 → 1013 — 2026-05-12: contracts/services/jellyfin.yaml's ``priority: 6`` for the new ensure-credentials job introduces a small literal; the ratchet's >100 sampling caught a sibling literal in admin_ops timeout paths. Net +1.  # 1013 → 1012 — ADR-0015 Phase 7i: ``ApplyScalePolicyRunner`` and ``DeployVerifyRunner`` lifted inline numeric literals onto named module constants. Net -1 magic-number literal. Tightened to the new floor.  # 1016 → 1013 — ADR-0015 Phase 7e: ``_run_serve`` had inline literals (``1800`` action timeout, ``9100`` default API port, ``600`` http-action timeout, ``900``/``1800``/``3600``/``21600``/``86400`` schedule intervals). The Phase 7e split lifts them onto named module constants (``_DEFAULT_API_PORT``, ``_DEFAULT_ACTION_TIMEOUT_SECONDS``, etc.) on the new SRP classes (``ControllerSchedulerSeedService`` consolidates 7 schedule intervals into one ``_DEFAULT_SCHEDULE_SEEDS`` tuple of dataclasses). Net -3 magic-number literals. Tightened to the new floor.  # 1019 → 1016 — ADR-0015 Phase 7c: deleting the 604-LoC ``run_controller_job_main.py`` retired inline literals (``120`` deadline, ``180`` timeouts, ``10`` urlopen timeout) that are now named constants (``_BOOTSTRAP_POD_DEADLINE_SECONDS``, ``_DEPLOYMENT_RESTART_TIMEOUT_SECONDS``, ``_BOOTSTRAP_TRIGGER_TIMEOUT_SECONDS``, ``_DEFAULT_POST_JOB_ACTION_TIMEOUT_SECONDS``) on the new orchestration sub-package. Tightened to the new floor.  # 1021 → 1019 — ADR-0015 Phase 4: deleting the two ``cli/commands/deploy_stack_runner_*.py`` mixin files removed two scattered numeric literals >100 (the validator's CHAOS_DURATION_MINUTES upper bound 120 and CHAOS_INTERVAL_SECONDS upper bound 3600 are now in :class:`DeployPhaseValidator`; one each appeared in the validator + one elsewhere in the deleted files). Baseline count was 1021 (pre-existing regression over the 1018 floor); locking in -2 at 1019.  # 1024 → 1023 — ADR-0005 Phase 5c.4c closure: deleted ``ControllerState.start_action(timeout_seconds=600)``, ``finish_action`` time-stamp + numeric literals went with it. Tightened to new floor.  # 1016 → 1024 — ADR-0005 Phase 5c.1 (wide): the two new ApiKeyDiscoverable wirers each declare four named constants in the >100 range — the canonical HTTP sentinels (200 / 401 / 403, used by ``_http_validate``) plus a 200-character error-message trim. The constant assignments themselves count even though the inline call sites now reference the names. # 1027 → 1003 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed many numeric literals >100. Tightened.
