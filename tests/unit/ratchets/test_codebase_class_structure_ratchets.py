...
FILES_OVER_400_LINES_RATCHET = 82  # 77 → 83 — ADR-0007 Phase 2 wave 5: six new route modules crossed 400 lines.   # 76 → 77 — ADR-0007 Phase 2 wave 3 sibling parallel-agent route module crossed 400 lines. NONE from the logs domain itself (`routes/logs.py` lands at ~190 lines).
HARDCODED_URLS_RATCHET = 152        # tightened 152 → 151 by the ADR-0005 Phase 3 Jellyseerr family cutover. ``JellyseerrConfigWirer`` composes ``_HTTPS_SCHEME_PREFIX`` from the ``_HTTPS_SCHEME = "https"`` constant rather than carrying a literal ``"https://"``, so the cutover added zero new URL literals. Net-improvement triggers a tightening per the ratchet-discipline rule.
