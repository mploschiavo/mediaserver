...
MODULES_WITHOUT_CLASS_RATCHET = 15   # 43 → 15 — ADR-0012 waves 4-9: 36 module-class extractions across api/services, application/, services/, infrastructure/ collapsed loose-function modules into class-based services with `_INSTANCE = MyService()` module-level singletons (uppercase ``_INSTANCE`` so the SINGLETON_INSTANCE_RATCHET regex on lowercase ``_instance = `` is unaffected). The legacy underscore + public alias surface is preserved on every file so callers and tests don't break.
...
DEEPLY_NESTED_4PLUS_RATCHET = 192         # 191 → 192 — ADR-0005 Phase 5c.4 closure: ``_action_loop`` retains the legacy ``while True: while True:`` retry shape inside ``_run_serve``; +1 deeply-nested branch. The retry semantics + error-cascade are preserved from the deleted subprocess loop.  # 193 → 189 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed deeply-nested elif-dispatch chains. Tightened to new floor.
...
GOD_CLASSES_OVER_500_LINES_RATCHET = 14  # 15 → 14 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed one god class >500 lines.
CLASSES_OVER_15_METHODS_RATCHET = 44  # 45 → 44 — ADR-0007 Phase E cleanup: deleted handlers_get/post.py removed one class with >15 methods. Tightened to new floor.
...
FILES_OVER_400_LINES_RATCHET = 91  # 92 → 91 — ADR-0010 Phase 7 cleanup: deleting ``api/services/lifecycle_ensurer_invoker.py`` (402 LoC, retired with the lifecycle-dispatch indirection) drops the over-400 file count by 1. Tightened to the new floor; "tighten on improvements" rule.
