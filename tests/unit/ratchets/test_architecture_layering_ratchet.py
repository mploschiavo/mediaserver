...
* Phase 16-B: migrate ``core/auth/`` to
  ``{domain,application,infrastructure,adapters}/auth/``. Shims
  remain at the legacy paths until Phase 16-F drops them.
"""
...
#
# ADR-0002 Phase 16-C dropped the two ``core/platforms/...`` entries:
# the source files were moved out of ``core/`` to ``adapters/`` (with
# 1-line shims left behind that no longer import from ``services/``).
# The adapters→services edge is allowed by the hexagon rules — only
# adapters→application is forbidden.
#
# ADR-0002 Phase 16-B dropped the remaining ``core/auth/...`` entry:
# ``configure_auth_job.py`` moved to
# ``application/auth/configure_auth_job.py`` (with a shim left behind
# that no longer imports from ``services/``). The
# ``services.runtime_platform`` import survives at the new location,
# but the ``application`` layer ratchet only forbids reaching into
# ``adapters/`` or ``infrastructure/`` — ``services/`` is permitted.
...
KNOWN_APPLICATION_VIOLATIONS: dict[str, set[str]] = {
    ...
    # ``adapters/defaults`` helper for loading the bootstrap-default
    # JSON blob. Phase 16-F either inverts this via a port or moves
    # the helper down into ``infrastructure/`` once the legacy flat
    # ``adapters/`` files are migrated.
    "application/jellyfin/home_rails_service.py": {
        "media_stack.adapters.defaults",
    },
    # ADR-0002 Phase 16-D batch 1 (jellyfin) — TODO(phase-16-F): the
    # Jellyfin runtime composition root pulls the SQLite api-key
    # reader directly from the new ``infrastructure/jellyfin/`` home.
    # Proper fix is a port in ``interfaces/`` plus dependency
    # injection at the bootstrap composition root; left as an
    # allowlist entry for now so the migration is mechanical.
