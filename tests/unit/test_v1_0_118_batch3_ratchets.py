...
Bug classes covered:
  ...
  #29  subprocess.run() always specifies the check= kwarg
       ...
       non-zero exit codes)
   #32 dist/ regen is byte-deterministic — running bin/regen-dist.sh
...
class DistRegenDeterminism(unittest.TestCase):
    """Running ``bin/regen-dist.sh`` twice in a row must produce
    ...
    def test_regen_is_byte_deterministic(self) -> None:
        ...
        for t in targets:
            if not t.is_file():
                self.skipTest(f"{t.name} not regen'd yet — run bin/regen-dist.sh")
        before = {str(t): hashlib.sha256(t.read_bytes()).hexdigest() for t in targets}
        try:
            subprocess.run(
                ["bash", "bin/regen-dist.sh"],
        ...
        self.assertFalse(
            drift,
            "bin/regen-dist.sh is not deterministic — these files "
