...
_DIST = ROOT / "deploy" / "dist" / "k8s-deploy.yaml"
# Phase 5 (ADR-0001) regrouped flat k8s/ manifests under k8s/base/<concern>/.
# Phase 6 (ADR-0001) moved the whole k8s/ tree under deploy/k8s/. The root
# kustomization at deploy/k8s/ remains the apply entry point and references
# files via relative ``base/<concern>/<file>`` paths.
_K8S_CONTROLLER = ROOT / "deploy" / "k8s" / "base" / "controller" / "controller.yaml"
_K8S_KUSTOMIZATION = ROOT / "deploy" / "k8s" / "kustomization.yaml"
_K8S_STANDARD_PROFILE = ROOT / "deploy" / "examples" / "bootstrap-profiles" / "media-k8s-standard.yaml"
