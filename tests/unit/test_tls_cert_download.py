...
class TrustCaWizardStepTests(unittest.TestCase):
    """Pin the wizard HTML so a future copy edit can't
    accidentally drop the trust-the-CA section."""
    ...
    def test_wizard_links_to_cert_download_endpoint(self) -> None:
        ...
        self.assertIn(
            "Trust the Stack Certificate", body,
            "Wizard must have an explicit 'Trust the Stack "
            "Certificate' section.",
        )

    def test_wizard_step_only_renders_under_https(self) -> None:
        """Pin the conditional. We don't want the trust step
        showing on HTTP-only stacks where there is no cert to
        trust."""
        idx = self.html.find("function renderWizardSteps")
        self.assertGreater(idx, -1)
        body = self.html[idx:idx + 10000]
        ...
        self.assertIn(
            ...
            "Step must check the page protocol so HTTP-only "
            "deploys don't see an irrelevant trust step.",
        )

    def test_wizard_has_per_os_instructions(self) -> None:
        """One-click download is half the value; the other half
        is telling users where to put the file. Pin the OSes we
        explicitly cover so a future trim doesn't quietly drop
        Linux or iOS."""
        idx = self.html.find("function renderWizardSteps")
        self.assertGreater(idx, -1)
        body = self.html[idx:idx + 10000]
        for token in ("macOS:", "Windows:", "Linux", "Firefox", "iOS"):
            self.assertIn(token, body,
                          ...
                          f"{token!r} install hint.")

    def test_wizard_does_not_reference_host_filesystem_path(self) -> None:
        """Old wording told users to copy
        ``docker/config/envoy/certs/cert.pem``. That's useless
        from the dashboard. Make sure the file path doesn't
        sneak back in."""
        idx = self.html.find("function renderWizardSteps")
        self.assertGreater(idx, -1)
        body = self.html[idx:idx + 10000]
        self.assertNotIn(
            ...
            "instead of a host filesystem path.",
        )
