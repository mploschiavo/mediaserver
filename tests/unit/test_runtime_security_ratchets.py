...
# #9 — dashboard endpoint registration parity
# ---------------------------------------------------------------------------
class DashboardEndpointParity(unittest.TestCase):
    """Every ``apiFetch('/api/...')`` in dashboard.html must have
    a matching handler registered in handlers_get.py or
    handlers_post.py — otherwise the UI button 404s silently.

    Soft-cap: handler discovery is best-effort; some endpoints
    are dynamically routed (/actions/<name>, /api/services/<id>/...)
    and we don't statically resolve them."""

    def test_dashboard_endpoints_have_backend_handlers(self) -> None:
        dash = (SRC / "api" / "dashboard.html").read_text(encoding="utf-8")
        get_src = (SRC / "api" / "handlers_get.py").read_text(encoding="utf-8")
        post_src = (SRC / "api" / "handlers_post.py").read_text(encoding="utf-8")
        server_src = (SRC / "api" / "server.py").read_text(encoding="utf-8")
        backend = get_src + "\n" + post_src + "\n" + server_src

        # Extract every /api/... path from apiFetch calls.
        # Skip template paths with ${...} or + concat — those resolve
        # at runtime.
        called: set[str] = set()
        pat = re.compile(r"""apiFetch\(\s*['"](/api/[^'"${}+]+)['"]""")
        for m in pat.finditer(dash):
            path = m.group(1)
            # Strip trailing dynamic suffix markers.
            called.add(path.rstrip("/"))

        missing: list[str] = []
        for ep in sorted(called):
            # Parent prefix match: ``/api/services/foo`` should match
            # ``/api/services/`` registered handler too. Use a
            # structural check: any registered handler that's a
            # prefix of the called endpoint.
            found = False
            for anchor in (f'"{ep}"', f"'{ep}'", f'{ep}"'):
                if anchor in backend:
                    found = True
                    break
            if not found:
                # Try prefix match — /api/X/Y → /api/X/ registered.
                parent = "/".join(ep.split("/")[:-1])
                while parent.startswith("/api"):
                    if f'startswith("{parent}/")' in backend or \
                       f"startswith('{parent}/')" in backend or \
                       f'"{parent}/"' in backend:
                        found = True
                        break
        ...
        self.assertFalse(
            missing,
            f"Dashboard calls {len(missing)} endpoint(s) with no "
            f"registered backend handler — clicks 404 silently:\n  - "
            + "\n  - ".join(missing[:20]),
        )
