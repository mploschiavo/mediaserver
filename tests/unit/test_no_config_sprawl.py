...
class TestBootstrapJobsReadFromContracts(unittest.TestCase):
    ...
    def test_no_config_json_import_in_bootstrap_jobs(self):
        """bootstrap_jobs.py must NOT load from JSON config files."""
        import inspect
        from media_stack.cli.commands import bootstrap_jobs
        source = inspect.getsource(bootstrap_jobs)
        self.assertNotIn("config.json", source, "bootstrap_jobs.py must not reference config.json")
        self.assertNotIn("_resolve_config_path", source, "bootstrap_jobs.py must not use _resolve_config_path")
