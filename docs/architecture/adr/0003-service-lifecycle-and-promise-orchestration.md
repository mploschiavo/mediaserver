...
- Phase 5: **shipped**. Staged rollout from shadow to primary via `ORCHESTRATOR_LIVE_SERVICES` env-var allowlist. 5a (v1.0.309): jellyfin → primary; auto-heal direct hook for `jellyfin:ensure-api-key` retired (orchestrator owns it). 5b/5c/5d: servarr family + remaining services flipped to primary; 16/16 services live. Coverage matrix at `docs/architecture/orchestrator-coverage-matrix.md` documents what the orchestrator covers vs what the bootstrap-phase code paths still own. Slice 5e (deletion of legacy paths) **partially shipped** — see audit at `docs/architecture/phase-5e-deletion-audit.md`: 5/6 originally-scoped paths still load-bearing for non-orchestrator flows (bootstrap, manual job invocation, cron). 5e.1 landed (assert-evaluator extracted to its own module so the legacy CLI is no longer on the orchestrator's runtime path). 5e.2 (delete the legacy `media-stack-probe-promises` CLI) is now unblocked: ADR-0004 Phase 6.4 shipped in v1.0.310 and `verify-fresh-install.sh` runs through the new verifier; deletion follows after one stable release of soak.
- Phase 6: **mostly shipped via ADR-0004** (v1.0.310). 6.1 (orchestrator state HTTP endpoint), 6.2 (`FreshInstallVerifier` class), 6.3 (`media-stack-verify` CLI), and 6.4 (`verify-fresh-install.sh` switchover) all landed. Operator and orchestrator now agree on "is the stack healthy?" by construction — same data, one runtime. 6.5 (legacy CLI deletion) is the soak-gated follow-up that completes 5e.2.
