...
- Phase 3: in progress — remaining `ServiceLifecycle` impls. Slice 3a (v1.0.297, **shipped**): `QbittorrentLifecycle` (session-cookie auth → password is the "key", honest mint-failure semantics that retire the silent-error-as-ok bug class) + `SabnzbdLifecycle` (INI variant of the *arr "wait for the file" pattern). Slice 3b (pending): Bazarr + Jellyseerr + Maintainerr. Slice 3c (pending): auth services + no-API-key services (Authelia, Authentik, Homepage, FlareSolverr, Envoy).
