...
- Phase 4: in progress. Slice 4a (v1.0.300, **shipped**): promise types in `domain/services/promises.py` (Promise, 8-kind ProbeSpec, 4-kind EnsurerSpec discriminated unions) + `infrastructure/promises/registry.py` YAML loader handling both legacy string-ensurer and new lifecycle-typed schemas. Schema ratchet asserts lifecycle resolution + acyclic depends_on graph. First two lifecycle-shaped promises landed (`jellyfin-running`, `jellyfin-api-key-discoverable`) as end-to-end proof. 30 new unit tests; 169 ADR-0003 tests total. Slices 4b (orchestrator core), 4c (shadow-mode hookup + first runtime deploy), 4d (discrepancy fixes from live data) pending.
