...
- Phase 4: in progress. Slice 4a (v1.0.300, **shipped**): types + loader + schema ratchet. Slice 4b (v1.0.301, **shipped**): orchestrator core + dispatcher + cooldown + CLI. Slice 4c (v1.0.302, **shipped**): `orchestrator:satisfy-shadow` job handler + auto-heal hookup at 60s cadence, `dry_run=True` so probes fire but ensurers don't conflict with legacy. First runtime change since v1.0.294. 7 new unit tests; 221 ADR-0003 tests total. Slice 4d (discrepancy fixes from live data) pending.
