...
1. **Git-tracked manifests and configs**
   ...
   - `contracts/promises.yaml` — post-install promises registry
...
- Promises in `contracts/promises.yaml` survive `compose down -v` because the meta-ratchet enforces every promise traces back to a real handler.
