...
If the new service ships a guarantee (e.g. "MyService has its API key configured after a fresh install"), add an entry to `contracts/promises.yaml` and re-render the reference. See [promises-registry.md](promises-registry.md) for the full procedure.
