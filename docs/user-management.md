...
| `CONTROLLER_SUDO_EXTRA_PATHS` | (unset) | Comma-separated extra paths that require re-auth (`X-Sudo-Password` header) when accessed via session cookie or bearer token. Extends the default set (`/api/rotate-keys`, `/api/reset-password`, `/api/users/*/reset-password`, `/api/users/*/delete`, etc.). Basic-auth requests skip this check \u2014 the password is already on every call. |
| `WEBHOOK_HMAC_SECRET` | (unset) | If set, `/webhooks/arr` verifies the GitHub-style `X-Hub-Signature-256: sha256=<hex>` header against this secret using constant-time HMAC-SHA256 compare. Unset = accept everything (legacy / private-network deployments). |
