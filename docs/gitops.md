...
1. Branch from main.
2. Update declarative config (`k8s/`, `bootstrap/`, `scripts/` as needed).
3. Run local tests:
```bash
...
```bash
bash scripts/verify-flow.sh media-stack-dev
bash scripts/microk8s-smoke-test.sh <NODE_IP> media-stack-dev
```
