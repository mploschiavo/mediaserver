...
def parse_config(argv: list[str] | None = None) -> ResetQbitWebuiAuthConfig:
    parser = argparse.ArgumentParser(
        prog="scripts/reset-qbit-webui-auth.sh",
        description=(
...
def _wait_until_no_pods(kubectl: list[str], cfg: ResetQbitWebuiAuthConfig, seconds: int = 60) -> None:
...
""",
    ...
    run_command([*kubectl, "-n", cfg.namespace, "scale", f"deploy/{cfg.deployment}", "--replicas=0"])
    _wait_until_no_pods(kubectl, cfg)

    print(f"[INFO] Scaling deploy/{cfg.deployment} back to 1")
    run_command([*kubectl, "-n", cfg.namespace, "scale", f"deploy/{cfg.deployment}", "--replicas=1"])
