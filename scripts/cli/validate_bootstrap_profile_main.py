...
import yaml
from core.bootstrap_profile import BootstrapProfileConfig
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="scripts/validate-bootstrap-profile.sh",
        description="Validate media-stack bootstrap profile YAML",
    )
    parser.add_argument(
        "--config",
        default="bootstrap/media-stack.bootstrap.yaml",
        help="Path to bootstrap profile YAML",
    )
    parser.add_argument(
        "--schema",
        default="bootstrap/media-stack.bootstrap.profile.schema.json",
        help="Path to bootstrap profile schema",
    )
    ...
    try:
        BootstrapProfileConfig.from_dict(payload, source_path=config_path)
