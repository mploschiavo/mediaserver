"""Render Traefik file-provider runtime config from compose service labels."""
...
_TRAEFIK_ENABLE_KEY = "traefik.enable"
_TRAEFIK_ROUTER_PREFIX = "traefik.http.routers."
_TRAEFIK_SERVICE_PREFIX = "traefik.http.services."
_TRAEFIK_MIDDLEWARE_PREFIX = "traefik.http.middlewares."
...
class TraefikDynamicConfigRender:
...
@dataclass
class TraefikDynamicConfigService:
    ...
    def render(self, services: dict[str, dict[str, Any]]) -> TraefikDynamicConfigRender:
        ...
        for service_name, spec in services.items():
            ...
            if not _truthy(labels.get(_TRAEFIK_ENABLE_KEY)):
                continue

            router_groups = self._parse_label_group(labels, _TRAEFIK_ROUTER_PREFIX)
            ...
            service_groups = self._parse_label_group(labels, _TRAEFIK_SERVICE_PREFIX)
            for traefik_service_name, fields in service_groups.items():
                service_owner.setdefault(traefik_service_name, service_key)
                raw_port = str(fields.get("loadbalancer.server.port") or "").strip()
                if raw_port:
                    try:
                        service_ports[traefik_service_name] = int(raw_port)
                    except Exception:
                        continue
                raw_scheme = str(fields.get("loadbalancer.server.scheme") or "").strip().lower()
                if raw_scheme:
                    service_schemes[traefik_service_name] = raw_scheme
                raw_pass_host = fields.get("loadbalancer.passhostheader")
                if raw_pass_host is not None:
                    service_pass_host_header[traefik_service_name] = _truthy(raw_pass_host)

            middleware_groups = self._parse_label_group(labels, _TRAEFIK_MIDDLEWARE_PREFIX)
        ...
        for traefik_service_name in sorted(service_owner.keys()):
            port = service_ports.get(traefik_service_name)
            if not port:
                continue
            owner_service = service_owner.get(traefik_service_name, "")
            target_container = container_name_by_service.get(owner_service)
            if not target_container:
                continue
            scheme = service_schemes.get(traefik_service_name, "http")
            load_balancer: dict[str, Any] = {
                "servers": [{"url": f"{scheme}://{target_container}:{port}"}],
            }
            if traefik_service_name in service_pass_host_header:
                load_balancer["passHostHeader"] = service_pass_host_header[traefik_service_name]
            http_services[traefik_service_name] = {"loadBalancer": load_balancer}
        ...
        return TraefikDynamicConfigRender(
            payload=payload,
