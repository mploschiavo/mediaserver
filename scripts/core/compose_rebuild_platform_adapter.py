...
class ComposeRebuildPlatformAdapter:
    ...
    def _is_traefik_service(self, labels: dict[str, str]) -> bool:
        enabled = str(labels.get("traefik.enable", "") or "").strip().lower()
        return enabled in {"true", "1", "yes", "on"}

    def _route_strategy(self) -> str:
        strategy = str(self.cfg.route_strategy or "").strip().lower()
        if strategy in {"path-prefix", "hybrid", "subdomain"}:
            return strategy
        return "subdomain"
    ...
    def _apply_edge_routing_labels(self, service_name: str, labels: dict[str, str]) -> None:
        if not self._is_traefik_service(labels):
            return
        strategy = self._route_strategy()
        service_key = str(service_name).strip().lower()
        is_jellyfin = service_key in {"jellyfin", "jellyfin-nvidia"}
        gateway_host = str(self.cfg.app_gateway_host or "").strip()

        if strategy == "path-prefix" and gateway_host and not is_jellyfin:
        ...
        if strategy in {"path-prefix", "hybrid"} and gateway_host:
            ...
            labels[f"traefik.http.routers.{path_router}.rule"] = (
                f"Host(`{gateway_host}`) && PathPrefix(`{path_prefix}`)"
            )
            labels[f"traefik.http.routers.{path_router}.service"] = service_name
            labels[f"traefik.http.middlewares.{strip_name}.stripprefix.prefixes"] = path_prefix
            self._apply_router_middleware(labels, path_router, strip_name)

        if is_jellyfin:
            direct_host = str(self.cfg.media_server_direct_host or "").strip()
            if direct_host:
                labels["traefik.http.routers.jellyfin.rule"] = f"Host(`{direct_host}`)"
    ...
    def _auth_middleware(self) -> str:
        ...
        if provider == "authelia":
            return "authelia@docker"
        if provider == "authentik":
            return "authentik@docker"
        return ""
    ...
    def _apply_auth_labels(self, service_name: str, labels: dict[str, str]) -> None:
        ...
        if not middleware:
            return
        service_key = str(service_name).strip().lower()
        if service_key in {"jellyfin", "jellyfin-nvidia"}:
            # Jellyfin keeps direct-app connectivity with native auth for TV/mobile clients.
            return
    ...
    def apply_environment_definition(self) -> None:
        ...
        self.info(
            ...
            f"auth_provider={self.cfg.auth_provider or 'none'}"
        )
