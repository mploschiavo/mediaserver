...
def normalize_text_list(values: Any, fallback: list[str] | None = None) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    source: list[Any]
...
def extract_epub_cover_bytes(epub_path: Path) -> bytes | None:
...
def iter_sidecar_books_root_candidates(sidecar_cfg: dict[str, Any]) -> list[Path]:
...
def iter_sidecar_music_root_candidates(sidecar_cfg: dict[str, Any]) -> list[Path]:
    raw_candidates: list[str] = []
...
def path_has_epub(path: Path) -> bool:
    try:
...
def path_has_audio(path: Path, audio_extensions: set[str]) -> bool:
    try:
...
def resolve_books_root_path(service, sidecar_cfg: dict[str, Any]) -> tuple[Path | None, list[Path]]:
...
def resolve_music_root_path(service, sidecar_cfg: dict[str, Any]) -> tuple[Path | None, list[Path]]:
...
def ensure_book_sidecar_artwork(service, prewarm_cfg: dict[str, Any]) -> None:
    d = service.deps
...
def ensure_music_sidecar_artwork(service, prewarm_cfg: dict[str, Any]) -> None:
    d = service.deps
