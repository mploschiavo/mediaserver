...
def coerce_exclude_name_tokens(raw_tokens: Any) -> list[str]:
...
def reputation_key(implementation: str, name: str) -> str:
    return f"{implementation}::{name}".lower()


def load_reputation_state(path: Path) -> dict[str, Any]:
    if path.exists():
        try:
...
def save_reputation_state(service, path: Path, state: dict[str, Any]) -> bool:
    try:
