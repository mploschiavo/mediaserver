"""Operations extracted from DiscoveryListsService."""
...
def build_arr_import_list_payload(
    ...
    root_folder_path = str(list_cfg.get("root_folder_path") or "").strip() or str(app_cfg.get("root_folder") or "").strip()
    if root_folder_path:
...
def ensure_arr_discovery_lists_for_app(
    ...
    for list_cfg in list_defs:
        ...
        if "signIn" in schema_fields:
            access_token = str(
                service.resolve_env_placeholder(((list_cfg.get("field_overrides") or {}).get("accessToken", "")))
            ).strip()
