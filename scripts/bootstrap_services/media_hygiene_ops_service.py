...
class MediaHygieneOpsService:
    ...
    def run_qbit_duplicate_prune(
        ...
        for group_items in groups.values():
            ...
            for candidate in sorted_items[1:]:
                ...
                if (
                    not candidate_hash
                    or candidate_hash == keep_hash
                    or candidate_hash in delete_seen
                ):
                    continue
    ...
    def run_qbit_ipfilter_refresh(
        ...
        apply_existing_on_failure = self.bool_cfg(
        ...
        min_refresh_interval_hours = (
            self.to_float(ipf_cfg.get("min_refresh_interval_hours"), 24.0) or 24.0
        )
    ...
    def run_qbit_queue_guardrails(
        ...
        enabled = self.bool_cfg(queue_cfg, "enabled", False)
        summary = {
            "enabled": enabled,
            "dry_run": self.bool_cfg(queue_cfg, "dry_run", False),
            "total": 0,
        ...
        count_states = {
            str(x).strip().lower()
            for x in self.coerce_list(queue_cfg.get("count_states"))
            if str(x).strip()
        ...
        prune_states = {
            str(x).strip().lower()
            for x in self.coerce_list(queue_cfg.get("prune_states"))
            if str(x).strip()
        } or {x.lower() for x in default_prune_states}

        include_uncategorized = self.bool_cfg(queue_cfg, "include_uncategorized", False)
        default_max_queued = self.to_int(queue_cfg.get("default_max_queued"))
        prune_when_over_limit = self.bool_cfg(queue_cfg, "prune_when_over_limit", True)
        over_limit_delete_files = self.bool_cfg(queue_cfg, "over_limit_delete_files", True)
        over_limit_max_delete_per_category = self.to_int(
        ...
        stale_max_stalled_hours = self.to_float(stale_cfg.get("max_stalled_hours"), 24.0) or 24.0
        stale_max_eta_seconds = self.to_int(stale_cfg.get("max_eta_seconds"), 14 * 24 * 3600)
        stale_min_progress = self.to_float(stale_cfg.get("min_progress"), 0.98)
        if stale_min_progress is None:
            stale_min_progress = 0.98
        stale_max_download_speed_bps = self.to_int(stale_cfg.get("max_download_speed_bps"), 32768)
        stale_max_delete_per_run = self.to_int(stale_cfg.get("max_delete_per_run"), 25)
        if stale_max_delete_per_run is None or stale_max_delete_per_run <= 0:
            stale_max_delete_per_run = 25
        stale_delete_files = self.bool_cfg(stale_cfg, "delete_files", True)
        stale_states = {
            str(x).strip().lower()
            for x in self.coerce_list(stale_cfg.get("states"))
            if str(x).strip()
        } or set(prune_states)

        opener = self.qbit_login(qbit_url, qb_username, qb_password)
        torrents = self.qbit_list_torrents(opener, qbit_url, "all")
        ...
        def parse_record(item: dict[str, Any]) -> dict[str, Any] | None:
            ...
            progress = self.to_float(item.get("progress"), 0.0) or 0.0
            added_on = self.to_int(item.get("added_on"), 0) or 0
            completion_on = self.to_int(item.get("completion_on"), 0) or 0
            last_activity = self.to_int(item.get("last_activity"), 0) or 0
            dlspeed = self.to_int(item.get("dlspeed"), 0) or 0
            eta = self.to_int(item.get("eta"), -1) or -1
            reference_on = completion_on if completion_on > 0 else added_on
            age_hours = 0.0
            if reference_on > 0:
                age_hours = max(0.0, float(now - reference_on) / 3600.0)
        ...
        if stale_to_delete:
            ...
            self.log("[OK] qB queue guardrails: no stale/slow torrent pruning required.")
        return summary
